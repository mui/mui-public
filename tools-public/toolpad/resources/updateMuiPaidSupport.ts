...
async function updateGitHubIssueLabels(repo, issueId) {
  if (!process.env.GITHUB_MUI_BOT2_PUBLIC_REPO_TOKEN) {
    throw new Error(
      "Env variable GITHUB_MUI_BOT2_PUBLIC_REPO_TOKEN not configured"
    );
  }
...
export async function updateMuiPaidSupport(
  issueId: string,
  repo: string,
  supportKey: string
) {
  ...
  if (targetSupportKeyExpirationDate < today) {
    return {
      message: `You support key expired on ${dayjs(
        targetSupportKeyExpirationDate
      ).format("MMMM D, YYYY")}.`,
    };
  }
