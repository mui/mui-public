...
`useDemo` inherits the [`useCode` editing behavior](../use-code/page.mdx#editing-behavior): collapsed snippets are editable, every loaded file stays selectable, and expanding is immediate. With `resetOnExpand: true`, expanding discards edits in every variant and file owned by the demo controller but keeps the current selection.
