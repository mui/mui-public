...
2. Each `useTypes()` call registers its types via a `registerTypes` callback, including both the namespaced name (e.g., `Accordion.Root.Props`) and any aliases (e.g., the flat export name `AccordionRootProps`)
...
4. `useType(name)` reads from the map to look up individual types — both namespaced and flat names work
