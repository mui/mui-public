...
The `useTypeProp` hook returns `TypePropData` containing the `ProcessedProperty` and its `href` anchor (e.g., `#root:class-name`). Properties are automatically registered by `useTypes` for:
