...
This hook assumes a **page-level layout**: it always compensates `window` scroll, and the offscreen check it uses to decide between anchors is measured against the window viewport. It does not currently expose a `scrollContainerRef` — if your code block lives inside its own scroll container, drop down to [`useScrollAnchor`](../use-scroll-anchor/page.mdx).
...
The single generic types the **toggle** element (`HTMLButtonElement` here), not the container — unlike [`useScrollAnchor`](../use-scroll-anchor/page.mdx), where the first generic types `containerRef`. The container is always typed as `HTMLDivElement` because the hook needs to query into it.
