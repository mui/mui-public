...
1. **Picks the anchor for you.** Looks for an element matching `[data-frame-type="highlighted"], [data-frame-type="focus"]` inside the container (the convention used by the highlighter pipeline). Falls back to the toggle ref when no such frame exists, or when the highlighted frame is offscreen on collapse. If neither anchor is available, `anchorScroll` no-ops.
