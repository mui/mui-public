...
The example above borders a single `focus` region; the demos generalize the "visible" side to any focused frame type (`padding-top`, `padding-bottom`, `highlighted`) and treat the hidden `-unfocused` overflow frames as bordering too — see the [`CollapsibleCodeContent.module.css`](#collapsible-code-block:collapsible-code-content.module.css) source for the full selector.
...
When `focusFramesMaxSize` is set and an explicit focus region exceeds that limit, a visible window is taken from the start of the region, and the remaining overflow lines are marked as unfocused. This demo marks the `<form>` JSX with both `@highlight` and `@focus`, then uses `focusFramesMaxSize: 6` to show its first 6 lines when collapsed. Expanding reveals the full 10-line highlighted region.
