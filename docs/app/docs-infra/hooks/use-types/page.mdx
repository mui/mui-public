...
import { DemoUseTypesMultiNamespace } from './demos/multi-namespace';

<DemoUseTypesMultiNamespace.Title />

This demo tests handling of files with multiple namespace exports (both `export * as Button` and `export * as Checkbox`). Each component should correctly resolve its own DataAttributes (ButtonRootDataAttributes and CheckboxRootDataAttributes).

<DemoUseTypesMultiNamespace />

[See Demo](./demos/multi-namespace/)
...
- **[`abstractCreateTypes`](../../functions/abstract-create-types/page.mdx)**: Factory functions where type processing happens
- **[`typesToJsx`](../../functions/abstract-create-types/page.mdx#typestojsx)**: Function that converts HAST to React nodes
