...
When deploying internally, use `isPrivate` to include pages with `audience: 'private'` in both the search index and default results:

```tsx
const { search } = useSearch({
  sitemap: () => import('./sitemap'),
  isPrivate: process.env.NEXT_PUBLIC_DEPLOYMENT !== 'production', // Default: false
});
```

When `isPrivate` is `false` (the default), pages with `audience: 'private'` are excluded from the search index.
