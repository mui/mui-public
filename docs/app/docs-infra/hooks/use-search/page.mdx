...
### Excluding Private Pages

When deploying to a public-facing site, use `isPublic` to automatically exclude pages with `audience: 'private'` from both the search index and default results:

```tsx
const { search } = useSearch({
  sitemap: () => import('./sitemap'),
  isPublic: process.env.NEXT_PUBLIC_DEPLOYMENT === 'production', // Default: false
});
```

When `isPublic` is `false` (the default), all pages are indexed regardless of audience.
...
interface UseSearchOptions {
  ...
  /** Exclude pages with audience: 'private' from the search index */
  isPublic?: boolean;
