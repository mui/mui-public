...
2. A `ResizeObserver` watches the container. Each time it fires, the hook compares the anchor's new top to the recorded one and calls `window.scrollBy(0, delta)` to compensate. When `scrollContainerRef` is set, it scrolls that container instead and applies any delta the container can't absorb to the page — so a fixed-height container that isn't scrollable yet (its content hasn't exceeded its `max-height`) still keeps the anchor put instead of drifting until it becomes scrollable. Sub-pixel deltas (`Math.abs(delta) <= 0.5`) are ignored.
