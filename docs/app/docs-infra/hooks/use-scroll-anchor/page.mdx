...
Modern browsers ship a CSS feature called [scroll anchoring](https://developer.mozilla.org/docs/Web/CSS/CSS_scroll_anchoring) that handles a narrow set of cases for free.
...
A timeline of release events, with the most recent at the bottom under a "Currently viewing" divider. Click _Load earlier events_ to insert older entries above your current scroll position. Switch between the three anchoring strategies and toggle **Animate** to see how each one holds up:

- **No anchoring** — the events you were reading are shoved off-screen along with the rest of your view. Same behaviour as Safari today.
- **Browser scroll anchoring** — Chromium and Firefox catch the snap when _Animate_ is off, but lose your place mid-flight when it's on.
...
For a real example built on this pattern, see [`useCollapsibleCode`](../use-collapsible-code/page.mdx), which uses `useScrollAnchor` internally to keep code blocks anchored as they expand and collapse.
...
- Expand / collapse sections, accordions, and code blocks (see [`useCollapsibleCode`](../use-collapsible-code/page.mdx), which uses this hook internally).
...
- [`useCollapsibleCode`](../use-collapsible-code/page.mdx) — built on top of this hook to keep collapsed code blocks anchored as they expand.
