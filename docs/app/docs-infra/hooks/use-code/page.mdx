...
- `'interaction'` — warm the engine on hover (`pointerenter`) and commit (load + attach `contentEditable`) on focus or click. The speculative preload is suppressed, so a block the reader never engages stays plain text and fetches nothing.
...
> [`CoordinatedLazy`](../../components/code-highlighter/page.mdx) / `useStream`
> primitives expose the same `refresh()` plus an opt-in `revalidateOnIdle` that
> automatically revalidates once the browser goes idle after the initial load.
