...
<CodeHighlighter
  sourceEnhancers={[addLineAnnotations]}
  Content={MyCodeContent}
>
  {code}
</CodeHighlighter>
...
function CodeWithDynamicHighlighting(props) {
  ...
  return (
    <div>
      <button onClick={() => setHighlightedLines([1, 2, 3])}>
        Highlight first 3 lines
      </button>
      {code.selectedFile}
    </div>
  );
...
<CodeHighlighter
  sourceEnhancers={[serverEnhancer]}
  Content={MyCodeContent}
>
  {code}
</CodeHighlighter>
