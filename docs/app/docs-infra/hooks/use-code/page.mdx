...
### Collapsed and expanded editing

Some demos only show a few lines of a file when collapsed. Editing that collapsed view
works the same as editing the full file: the hook writes your change back into the right
place in the complete file before handing it to the controller. Blank lines shown around
the snippet can be edited too. If the visible lines cannot be mapped to one continuous
range of the file, the collapsed view shows the complete file instead.

Expanding shows the complete file right away, with no resize or fade animation. Collapsing
goes back to the short view. Every file stays available in both views, including
JavaScript, TypeScript, CSS, and CSS Modules.

Set `resetOnExpand: true` to discard all edits when the demo expands. This resets every
variant and file but keeps the selected variant, file, and transform. It applies however
the demo was expanded: a toolbar button, keyboard navigation, or a link to the source.

### Editor interaction

The editor is a single tab stop. Press <kbd>Enter</kbd> to start editing, <kbd>Escape</kbd>
to leave the editor, and <kbd>Tab</kbd> to indent. Clicking or tapping starts editing
directly. Read-only blocks never load the editor.

Starting to edit a live demo switches it to the live preview immediately, even before the
first change. It stays on the live preview until the demo unmounts; resetting the source
does not switch back to the bundled component.

### Selected file metadata

The hook exposes details about the selected file so controls do not need to inspect the
rendered code:

- `selectedFileHasFocusProjection`: the collapsed view shows a snippet of the file.
- `selectedFileCollapsible`: the collapsed and expanded views differ.
- `selectedFileFocusedLines` and `selectedFileLines`: line counts of the snippet and the full file.
- `selectedFileOriginalName`: the filename in the repository. `selectedFileName` is the displayed name, which a transform may change.
- `hasControlledEdits`: the controller holds edited source.

### Source actions

`actionSource` picks which source `copy`, `copyMarkdown`, and the export actions from
`useDemo` use:

- `'current'` (default): the edited source, matching the existing playground behavior.
- `'initial'`: the repository source, ignoring edits. The selected variant and transform
  still apply.

Use `'initial'` when copy and export should stay stable while the reader experiments:
...
Pass `selectedTransform` with `onSelectedTransformChange` to store the transform choice
yourself, for example in a preference store. The URL still wins on first load, but the
hook's own local storage no longer overrides your value. Selecting a transform calls your
handler; the display updates when your value changes.
...
Uncontrolled consumers can keep using `initialTransform` and the stored preference.

### Switching transforms and variants

Switching keeps the current code on screen until the new one is ready, then swaps once
...
- **useSourceEnhancing**: Applies enhancer functions to parsed HAST sources
...
When the code is rendered through a controller that supplies a `setSource` handler (see [`CodeControllerContext`](../../components/code-controller-context/page.mdx)), the editor uses an explicit keyboard engagement step so normal page tab navigation is preserved. See [Editable Code Interaction](../../components/code-controller-context/page.mdx#editable-code-interaction) for the interaction details.

### Live editing is loaded on demand

The live editor and asynchronous highlighting support are loaded **on demand**, so read-only code blocks never ship the editor. An editable block loads it once editing activates: an eager [`CodeProvider`](../../components/code-provider/page.mdx) can bundle it for zero-latency editing, while [`CodeProviderLazy`](../../components/code-provider/page.mdx) fetches it once per page. Until the editor resolves, the block stays selectable, read-only text.

Use the `editActivation` prop — set on [`CodeHighlighter`](../../components/code-highlighter/page.mdx) or, more commonly, on a demo created with the [demo factory](../../components/code-highlighter/page.mdx#demo-factory-pattern) — to defer the load until the reader actually engages the code:

- `'eager'` (default) — load the editor as soon as the block is editable, and let `CodeHighlighter` speculatively preload it on first render.
- `'interaction'` — warm the editor on hover (`pointerenter`) and commit on focus or pointer down. The speculative preload is suppressed, so a block the reader never engages stays plain text and fetches nothing.

```tsx
// Editing activates only once the reader hovers, focuses, or clicks the code.
<CodeHighlighter editActivation="interaction" {...props} />
```

A related nicety — **boundary auto-expand**: in a collapsed
[collapsible block](../use-code-window/page.mdx), the caret respects the
collapsed-frame boundaries while editing, and moving past them (e.g. pressing <kbd>↓</kbd>
...
- **`editable`** — whether edit mode is on right now. While it's `false` the block stays
  read-only and the live-editing engine isn't even warmed.
- **`setEditable(next)`** — flips edit mode. It is `undefined` when the block can't be
  edited at all (no controller, or a hard `disabled`), so the button only renders where a
  toggle is meaningful.
- Turning editing **on** hands off to [`editActivation`](#live-editing-is-loaded-on-demand)
  (the engine loads eagerly or on the next interaction); turning it **off** keeps the
  reader's edits — use `reset()` to discard them.

Start a block read-only with **`initialDisabled`** (a truthy value starts it read-only),
resolved exactly like `initialExpanded` — instance prop → demo `meta` → factory default:

- **All demos** — in the [`createDemoFactory`](../../components/code-highlighter/page.mdx#demo-factory-pattern)
  ...
  ```
- **Per demo** — in the `createDemo` call's `meta` (overrides the factory default; pass
  `false` to opt a demo back in):
  ```ts
  export const meta = { initialDisabled: true };
  ```
- **Per render** — as a prop on a rendered demo: `<Demo initialDisabled />`.

With none set, blocks stay editable. `initialDisabled` is distinct from `disabled`:
...
import { TypesUseCode } from './types';
...
`refresh()` re-runs the **full** variant loader (`loadIsomorphicCodeVariant`),
...
rendered without a [`CodeProvider`](../../components/code-provider/page.mdx). On
...
- **`copy`** — copies the **selected file's** plain text.
- **`copyMarkdown`** — copies the **entire variant** as a Markdown snippet: the
  ...
  an issue, a chat message, or an LLM prompt.
...
Both are wired through [`useCopier`](../use-copier/page.mdx); pass the `copy`
...
> [`CoordinatedLazy`](../../components/coordinated-lazy/page.mdx) /
> [`useStream`](../use-stream/page.mdx) primitives expose the same `refresh()`
> plus an opt-in `revalidateOnIdle` that automatically revalidates once the
> browser goes idle after the initial load.
...
- Derivation of `selectedFileUrl` by resolving the selected file's name (or its `relativeUrl`) against the variant's `url`. Available on the `useCode` result so consumers can link back to the source file (e.g., for an "Edit on GitHub" link).
...
- **`CodeHighlighter`** component via the `sourceEnhancers` prop (runs on server or during hydration)
- **`useCode`** hook via the `sourceEnhancers` option (runs on client after hydration)

### Enhancer Basic Usage

```tsx
import type { SourceEnhancer, HastRoot } from '@mui/internal-docs-infra/CodeHighlighter/types';
...
> [!TIP]
> When passing state to enhancers, include the state values in the `useMemo` dependency array. This ensures the enhancer is re-created when state changes, triggering re-enhancement of the code.
...
- Static enhancements (line highlighting, syntax annotations)
- Comment-driven enhancements (e.g., `// @highlight`)
- Any transformation that doesn't depend on client state

**When to use server-time enhancers:**

- Custom enhancer logic not available as a built-in configuration
- Enhancements that need access to server-side data (e.g., database lookups)
- Dynamic routes where build-time precomputation isn't possible

**When to use client-time enhancers:**

- **Lazy loading**: Defer expensive enhancements until needed
- **Personalization**: Apply user-specific annotations or themes
- **Live editing**: Re-enhance code when users modify source in interactive editors
- **Dynamic context**: Enhancements that depend on runtime state

> [!NOTE]
> The [Precompute Loader](../../pipeline/load-precomputed-code-highlighter/page.mdx) only supports **configurable** built-in enhancers—you cannot pass custom enhancer functions directly. For custom enhancer logic at build/server time, pass `sourceEnhancers` to the [`CodeHighlighter`](../../components/code-highlighter/page.mdx) component in a Server Component. Client-time enhancers are passed through `useCode` options.
...
This constraint is why comments extracted with `notableCommentsPrefix` are removed from the source _before_ the plain text version is generated. If comment removal happened in an enhancer instead, the line numbers would shift and cause a jarring visual jump when the enhanced code replaces the plain text placeholder.

**Use enhancers for:**

- Adding `data-*` attributes (e.g., `data-hl` for line highlighting)
- Wrapping tokens in additional `<span>` elements
- Adding CSS class names to existing elements
- Injecting metadata into the HAST properties

**Use transformers for:**

- Creating **alternative representations** of the code (e.g., TypeScript → JavaScript)
- Generating optional variants users can switch between

> [!NOTE]
> **Transformers are for alternatives, not modifications.** Transformers create additional views of the code that users can toggle between—they should not be used to alter the default version. If you need to modify the source itself (e.g., fixing formatting, removing debug code), do that in a linter or build step _before_ the code reaches `CodeHighlighter`.

See the [Code Highlighter transforms documentation](../../components/code-highlighter/page.mdx#code-transformed) for details on using transformers.

### Important Considerations

> [!WARNING]
> **Stable References Required**: Always use `React.useMemo` or define enhancer arrays outside your component to prevent infinite re-render loops. Creating new arrays on each render will cause the hook to continuously re-run enhancement.
