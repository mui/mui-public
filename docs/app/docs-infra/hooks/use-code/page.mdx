...
When the code is rendered through a controller that supplies a `setSource` handler (see [`CodeControllerContext`](../../components/code-controller-context/page.mdx)), the `<Pre>` rendered by `code.selectedFile` adds a keyboard focus trap so that <kbd>Tab</kbd> through the page doesn't get caught by contentEditable's indent behavior. See [Editable Code Focus Trap](../../components/code-controller-context/page.mdx#editable-code-focus-trap) for the interaction details and the CSS hooks you can style.

### Live editing is loaded on demand

The heavy live-editing runtime — the `contentEditable` setup and the keystroke/caret/paste handling (the `EditableEngine` chunk, a few KB gzip, with `react-dom` left external) — is loaded **on demand**, so read-only code blocks never ship it. An editable block loads it once the block renders: an eager [`CodeProvider`](../../components/code-provider/page.mdx) bundles it for zero-latency editing, while [`CodeProviderLazy`](../../components/code-provider/page.mdx) (the general default) fetches it once per page (deduped), and `CodeHighlighter` preloads it as soon as it detects an editable `CodeControllerContext` (one that exposes `setCode`). Until the engine resolves, `contentEditable` is not applied — the block stays selectable, read-only text.
...
- `'eager'` (default) — load the engine as soon as the block is editable, and let `CodeHighlighter` speculatively preload it on first render.
- `'interaction'` — warm the engine on hover (`pointerenter`) and commit (load + attach `contentEditable`) on focus or pointer down. The speculative preload is suppressed, so a block the reader never engages stays plain text and fetches nothing.
...
### Headless editing, for a host with its own editor

A host that already has an editor — its own DOM, styling, focus hints, and error
overlay — keeps it by setting `editorMode: 'headless'`. docs-infra then mounts no
editor of its own: the rendered file stays highlighted, read-only source, and the
host drives editing from the source state `useCode` returns.

```tsx
function HostEditor(props) {
  const code = useCode(props, { editorMode: 'headless' });

  return (
    <div>
      <textarea
        value={code.selectedFileProjection?.source ?? code.selectedFileSource ?? ''}
        onFocus={code.activateEditing}
        onChange={(event) => code.setSource?.(event.target.value)}
        readOnly={!code.selectedFileEditable}
      />
      {code.selectedFile}
    </div>
  );
...
- **`selectedFileSource`** — the file as plain text, decoded from whatever shape it
  arrived in: a string, HAST, serialized HAST, or a compressed payload decoded
  through the block's fallback dictionary. `null` in `'internal'` mode, where
  decoding would cost an inflate nothing reads.
- **`selectedFileProjection`** — the contiguous slice a collapsed view is edited
  through, or `undefined` when the whole file is directly editable. Show its
  `source` while collapsed.
- **`selectedFileLanguage`**, **`selectedFileOriginalName`** — the grammar and the
  canonical, pre-transform file name. `selectedFileName` is the displayed name,
  which a transform renames.
- **`selectedFileEditable`** — whether an edit would be accepted right now.
- **`activateEditing()`** — warms the edit-time engine, so the first edit applies
  without a flash. Call it when the host's surface takes focus.

Edits go back through the same `setSource` the internal editor uses, so the
controller, the reset behaviour, and the rendered preview all work unchanged.
