...
interface UseCodeOpts {
  ...
  fileHashMode?: 'full' | 'read' | 'remove'; // Controls URL hash management
}
```
...
The hook provides fine-grained control over URL hash management through the `fileHashMode` option, which offers three modes to suit different use cases:
...
**Read mode - Clean URLs for embedded viewers:**
...
- **Controlled Behavior**: Use `fileHashMode: 'read'` or `'remove'` to prevent hash updates while still reading initial state
...
A dedicated sub-hook that handles all URL hash management for file navigation. Controlled through the `fileHashMode` option with three distinct modes: `'full'` (default bidirectional sync), `'read'` (read-once with cleanup), and `'remove'` (read-once then remove). 
...
- Cleaning hashes after initial read: keeps demo slug in `'read'` mode, removes completely in `'remove'` mode
...
- **Hash not being cleaned**: Ensure `fileHashMode` is set to `'read'` or `'remove'` and the hash belongs to the current demo
- **Hash still updating on clicks**: Verify `fileHashMode` is set to `'read'` or `'remove'` (default is `'full'` which updates hashes)
...
- **Hash removed unexpectedly**: If using `fileHashMode: 'remove'`, the hash will be removed on mount - this is expected behavior
- **Hash persisting after navigation**: With `fileHashMode: 'read'`, the simplified demo hash is preserved; use `'remove'` if you want complete removal
