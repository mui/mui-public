...
> [!NOTE]
> The [Precompute Loader](../../functions/load-precomputed-code-highlighter/page.mdx) only supports **configurable** built-in enhancers—you cannot pass custom enhancer functions directly. For custom enhancer logic at build/server time, pass `sourceEnhancers` to the [`CodeHighlighter`](../../components/code-highlighter/page.mdx) component in a Server Component. Client-time enhancers are passed through `useCode` options.
