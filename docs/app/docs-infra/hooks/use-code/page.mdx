...
// Simple enhancer that adds a comment to the end of code
const addCommentEnhancer: SourceEnhancer = (root, comments, fileName) => {
  return {
    ...root,
    children: [...root.children, { type: 'text', value: '\n// Enhanced!' }],
  };
};

function MyCodeContent(props) {
  // Create stable reference to prevent infinite re-renders
  const enhancers = React.useMemo(() => [addCommentEnhancer], []);
...
### Build-Time vs Client-Time Enhancement
...
You can combine build-time and client-time enhancers—they apply at different stages:
