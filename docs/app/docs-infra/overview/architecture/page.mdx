...
types.md (already on disk)
    ...
    └────┬────┘
         │
         ▼
    EnhancedTypesMeta with HAST fields
```

Both paths produce identical `EnhancedTypesMeta` with syntax-highlighted HAST — the only difference is where the type data comes from.
