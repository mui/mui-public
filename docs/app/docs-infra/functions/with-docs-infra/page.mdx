...
The default filter `{ include: ['app/'], exclude: [] }` processes all pages in `app/`. Index files (e.g., `app/page.mdx`, `app/components/page.mdx`) are automatically excluded to prevent them from creating parent indexes. The base directory is automatically detected from `process.cwd()`.
