...
The default filter `{ include: ['app/'], exclude: ['app/page.mdx'] }` processes all pages in `app/` except the root page. The base directory is automatically detected from `process.cwd()`.
