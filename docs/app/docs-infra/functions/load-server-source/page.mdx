...
Strips comments starting with specified prefixes from the output source, while still including them in the returned `comments` object:
...
// result.comments: { 1: ['@internal This is an implementation detail'], 2: ['User-visible comment'] }
```
...
Marks comments with specified prefixes as notable (reserved for future use):

```ts
const loadSource = createLoadServerSource({
  notableCommentsPrefix: ['@highlight', '@section'],
});
```
