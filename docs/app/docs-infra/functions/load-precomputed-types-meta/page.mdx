...
- **HAST output format**: Types formatted as syntax-highlighted HAST nodes for efficient rendering
...
- **Auto-included metadata files**: Automatically discovers and includes `DataAttributes` and `CssVars` files from component directories
...
When a component has multiple parts (like `CheckboxRoot` and `CheckboxIndicator`), you can document them all at once by passing an object with multiple components:

```typescript
import { createMultipleTypes } from '../createTypes';
import { Checkbox } from '@base-ui/react/checkbox';

// Processes all parts of the Checkbox component
const types = createMultipleTypes(import.meta.url, Checkbox);

// Export individual type components for each part
export const TypesCheckboxRoot = types.Root;
export const TypesCheckboxIndicator = types.Indicator;
```

The factory function extracts the component structure and returns an object where each key corresponds to a component part. This allows you to export type documentation components individually while processing them all together for efficiency.

### Automatic Metadata File Discovery

The loader automatically discovers and includes `DataAttributes` and `CssVars` files from component directories. This ensures that data attributes and CSS custom properties are documented alongside component props without requiring manual configuration.

**File Structure:**

```
app/components/checkbox/
├── types.ts              # Your createTypesMeta() file
├── Checkbox.tsx          # Main component
├── CheckboxDataAttributes.ts  # ← Auto-discovered
└── CheckboxCssVars.ts         # ← Auto-discovered
```

**How It Works:**

1. The loader scans the directory containing your component file
2. It recursively searches for files ending with `DataAttributes.ts` or `CssVars.ts`
3. These files are automatically included as entrypoints for type analysis
4. The extracted types are merged with your component's documentation

**Example DataAttributes file:**

```typescript
/**
 * Data attributes applied to the Checkbox root element.
 */
...
The loader also discovers metadata files from re-exported components. For example, if your component re-exports from a parent directory:

```typescript
// app/components/autocomplete/Autocomplete.tsx
export { Combobox as Autocomplete } from '../combobox';
```

The loader will search `../combobox/` for `DataAttributes` and `CssVars` files and include them in the documentation.

This automatic discovery works for both directly defined components and re-exported components, ensuring comprehensive documentation without manual entrypoint configuration.

### Global Types Configuration

In most cases, global types are automatically detected. You only need to specify them manually if the loader suggests missing types:
...
Global types ensure the TypeScript compiler has access to necessary type definitions during analysis. The loader automatically detects most required types, but you can manually specify them if needed.

### Automatic Error Detection

If global types are missing, the loader provides helpful suggestions:
...
When manual specification is needed, these are the most common values:
...
You can also specify custom type packages like `'@types/lodash'` or `'@mui/types'` if needed.
...
interface LoaderOptions {
   ...
   * When using `withDocsInfra` in Next.js, this defaults to `.next/cache/docs-infra/types-meta-worker`.
   * Otherwise, falls back to `RUNNER_TEMP` (GitHub Actions), `AGENT_TEMPDIRECTORY` (Azure Pipelines), or system temp directory.
   */
  socketDir?: string;
}
```
...
The loader extracts comprehensive type information using [`typescript-api-extractor`](https://github.com/michaldudak/typescript-api-extractor/blob/main/README.md#output-format) and formats it with **[HAST (Hypertext Abstract Syntax Tree)](../../patterns/hast/page.mdx) nodes** for syntax-highlighted rendering.

### Type Metadata Format

The loader produces two layers of types:

**Build-time Format** (stored in JSON):

- Type fields (`type`, `shortType`, `detailedType`, `default`) are formatted as **HAST** with inline syntax highlighting
- Markdown fields (`description`, `example`) are parsed as **HAST** with embedded code blocks
- These HAST structures enable zero runtime parsing overhead

**Runtime Format** (after `useTypes` processing):

- HAST nodes are converted to `React.ReactNode` for rendering
- See [`useTypes`](../../hooks/use-types/page.mdx) for runtime conversion details

### Formatted Types (Build-time)

```typescript
interface FormattedProperty {
  /** Syntax-highlighted type as HAST (not a string) */
  type: HastRoot;
  /** Short simplified type for table display */
  shortType?: HastRoot;
  /** Plain text version of shortType */
  shortTypeText?: string;
  /** Default value with syntax highlighting as HAST */
  default?: HastRoot;
  /** Plain text version of default */
  defaultText?: string;
  ...
  example?: HastRoot;
  /** Detailed expanded type view (when different from basic type) */
...
> [!NOTE]
> All type and markdown fields are **HAST nodes**, not strings. This enables:
>
> - Syntax highlighting computed once at build time
> - Direct rendering without runtime parsing
> - Smaller server/client bundles (no highlighting libraries needed at runtime)

> [!TIP]
> **Multiline Union Formatting**: When union types in `shortType` or `default` fields exceed the configured width (default: 40 characters), they are automatically formatted across multiple lines with leading pipe characters (`| `). This improves readability in type tables. Configure the thresholds using `formatting.shortTypeUnionPrintWidth` and `formatting.defaultValueUnionPrintWidth` in the [Loader Options](#loader-options).

### TypeScript API Extractor Output

The raw TypeScript analysis produces structured metadata:
...
The loader can extract different types of exports. Here are examples of each:
                ...
                "children": [
                  {
                    ...
                    "children": [{ "type": "text", "value": "'primary' | 'secondary'" }]
                  }
                ]
              },
              "description": {
                ...
                "children": [
                  {
                    "type": "element",
                    "tagName": "p",
                    "children": [{ "type": "text", "value": "The visual style variant" }]
                  }
                ]
              },
              "optional": true
                ...
                "children": [
                  {
                    "type": "element",
                    "tagName": "p",
                    "children": [{ "type": "text", "value": "The initial toggle state" }]
                  }
                ]
              },
              "optional": true
...
> [!TIP]
> Notice how `type` and `description` fields contain HAST tree structures, not plain strings. This pre-computed format eliminates runtime parsing overhead and enables zero-cost rendering at runtime.
...
> [!NOTE]
> The loader automatically removes `Documentation: <url>` suffixes that may be added by API extraction tools. For example, `"Button component\n\nDocumentation: https://..."` becomes `"Button component"` in the generated documentation.

### Code Examples in Descriptions

You can include code examples using the `@example` JSDoc tag:
...
The `@example` tag is automatically extracted and parsed as markdown. Multiple `@example` tags can be used if needed.
...
[See Types](./types.md#loadPrecomputedTypesMeta)
...
The loader requires a valid `tsconfig.json` at `{rootContext}/tsconfig.json` (typically the workspace root). If the file cannot be found or read, the build will fail with a file system error.
...
The loader implements a multi-stage pipeline that transforms TypeScript types into renderable React components:

### Build Time (Webpack Loader)

1. **Extract TypeScript types**: Worker thread analyzes your components using TypeScript compiler API
2. **Format as HAST**: Main thread converts types to syntax-highlighted HAST nodes
3. **Store in JSON**: Precomputed HAST is embedded in your source code as JSON
...
1. **Load precomputed data**: Your component imports the factory function with embedded HAST
2. **Convert HAST to React**: [`useTypes`](../../hooks/use-types/page.mdx) hook converts HAST to React elements
3. **Render documentation**: Your TypesContent component displays the interactive docs

```typescript
// In your documentation page
import { TypesButton } from './types';

// TypesButton contains precomputed HAST data
// useTypes converts it to React.ReactNode
// Your TypesContent component renders it
<TypesButton />
```

### Why HAST?

HAST (Hypertext Abstract Syntax Tree) is an intermediate format that provides:

- **Build-time optimization**: Syntax highlighting computed once during compilation
- **Zero runtime parsing**: No need to parse markdown or apply syntax highlighting on every render
- **Smaller bundles**: No syntax highlighting libraries shipped to clients
- **Efficient caching**: HAST can be serialized to JSON and cached by webpack
- **Framework agnostic**: HAST can be converted to React, Vue, or other frameworks

**Type Flow:**

```
TypeScript Types → Worker Analysis → HAST (Build) → JSON → HAST (Load) → React.ReactNode (Runtime) → Display
```
...
- **Unix domain socket**: Uses `.next/cache/docs-infra/types-meta-worker` by default (when using `withDocsInfra`), with fallback to `RUNNER_TEMP` (GitHub Actions), `AGENT_TEMPDIRECTORY` (Azure Pipelines), or system temp directory
- **Configurable socket directory**: Override via `socketDir` loader option for manual configurations
...
> [!NOTE]
> When using `withDocsInfra`, the socket directory is automatically set to `.next/cache/docs-infra/types-meta-worker`, which works reliably across all platforms including Windows CI. For manual webpack configurations, use the `socketDir` loader option.
...
The main thread formats the raw exports into HAST structures:

- Initializes inline syntax highlighting via [Starry Night](https://github.com/wooorm/starry-night)
- Calls `formatComponentData()` for components
- Calls `formatHookData()` for hooks
- **Generates HAST nodes**: Converts types to syntax-highlighted HAST
- **Parses markdown**: Converts JSDoc descriptions to HAST
- Parallelizes formatting using `Promise.all()`

> [!NOTE]
> At this stage, all type fields (`type`, `shortType`, `detailedType`, `default`) and markdown fields (`description`, `example`) become **HAST nodes**, not strings. This precomputed format enables zero-cost rendering at runtime.
...
- **[`abstractCreateTypes`](../abstract-create-types/page.mdx)**: Factory functions for creating type documentation components
- **[`useTypes`](../../hooks/use-types/page.mdx)**: Hook for converting HAST types to React nodes at runtime
