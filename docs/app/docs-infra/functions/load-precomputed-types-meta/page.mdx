...
The loader processes `types.ts` files that use the `createTypesMeta` factory pattern, analyzing your TypeScript components and extracting detailed metadata about their APIs. It delegates the actual type processing to [`loadServerTypesMeta`](../load-server-types-meta/page.mdx), then injects the precomputed results back into your source code.
...
Calls [`loadServerTypesMeta`](../load-server-types-meta/page.mdx) with the parsed factory call, which handles:
...
- [`loadServerTypesMeta`](../load-server-types-meta/page.mdx) - Core type processing logic
