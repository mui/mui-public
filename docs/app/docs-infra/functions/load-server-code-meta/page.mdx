...
const code = await loadServerCodeMeta(
  'file:///app/components/form/demos/validation/index.ts',
);
...
```ts
function createLoadServerCodeMeta(
  options?: CreateLoadCodeMetaOptions,
): LoadCodeMeta
```
