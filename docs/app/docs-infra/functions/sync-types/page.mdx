...
> **Note**: Syntax highlighting is handled separately by [`loadServerTypes`](../load-server-types/page.mdx), which calls this function internally.
...
> **Note**: The `variantData` is not syntax-highlighted. Use [`loadServerTypes`](../load-server-types/page.mdx) if you need highlighted output ready for rendering.
