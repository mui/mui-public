...
> **Note**: The `baseDir` is needed because Next.js provides absolute file paths to remark plugins. The plugin strips this prefix to match against your include/exclude patterns.
...
const withMDX = createMDX({
  options: getDocsInfraMdxOptions({
    ...
    // { include: ['app/'], exclude: ['app/page.mdx'], baseDir: process.cwd() }
  }),
});
...
const withMDX = createMDX({
  options: getDocsInfraMdxOptions({
    extractToIndex: {
      ...
      exclude: ['app/page.mdx', 'app/docs/internal/'],
    },
  }),
});
...
- `true` - Enabled with default filter: `{ include: ['app/'], exclude: ['app/page.mdx'], baseDir: process.cwd() }`
- `{ include: string[], exclude: string[], baseDir?: string }` - Enabled with custom path filters

**Path matching** uses prefix matching - a file matches if its path starts with any include pattern and doesn't start with any exclude pattern.
...
.use(transformMarkdownMetadata, {
  extractToIndex: {
    ...
    exclude: ['app/docs/internal/', 'app/page.mdx'],
...
The default `{ include: ['app/'], exclude: ['app/page.mdx'] }` is designed for Next.js App Router:

- **Includes `app/`**: Processes all pages in the app directory
- **Excludes `app/page.mdx`**: Prevents creating a root `/page.mdx` file, which would conflict with your site's home page
- **Route groups**: Next.js route groups like `(shared)` are automatically removed when matching, so `app/(shared)/page.mdx` is treated as `app/page.mdx` and correctly excluded

This ensures index pages are created at every level except the root, providing web-native navigation without interfering with your site structure.
