...
> [!NOTE]
> For HAST generated at build time by this package (from `loadPrecomputedCodeHighlighter` or `loadPrecomputedTypes`), sanitization is not necessary since the content comes from your own source files.
...
- [`parseSource`](../../pipeline/parse-source/page.mdx): Converts source code to HAST with syntax highlighting using Starry Night

### Converting HAST

- [`hastToJsx`](../../pipeline/hast-utils/page.mdx): Converts HAST nodes to React elements
- [`hastOrJsonToJsx`](../../pipeline/hast-utils/page.mdx): Handles both HAST and serialized JSON formats
...
- [`transformHtmlCodePrecomputed`](../../pipeline/transform-html-code-precomputed/page.mdx): Processes `<pre><code>` blocks in HAST and precomputes syntax highlighting data
- [`transformHtmlCodeInlineHighlighted`](../../pipeline/transform-html-code-inline-highlighted/page.mdx): Applies inline syntax highlighting to HAST
- All `transformHtml*` functions work with HAST nodes

**Remark plugins** (operate on MDAST/Markdown AST):

- [`transformMarkdownCode`](../../pipeline/transform-markdown-code/page.mdx): Groups markdown code fences with variants and converts to semantic HTML/HAST
...
Enhancers are rehype plugins that add visual annotations to an existing HAST tree **without changing its plain text output**. They may add wrapper elements, CSS class names, `data-*` attributes, or metadata — but must never modify or remove text nodes.
...
- [`enhanceCodeInline`](../../pipeline/enhance-code-inline/page.mdx): Fixes tag bracket wrapping, reclassifies syntax tokens, and styles nullish values
- [`enhanceCodeExportLinks`](../../pipeline/enhance-code-export-links/page.mdx): Links type identifiers and properties to documentation anchors
- [`enhanceCodeEmphasis`](../../pipeline/enhance-code-emphasis/page.mdx): Adds emphasis styling to code elements
...
- **Code highlighting**: Passed as `sourceEnhancers` to [`CodeHighlighter`](../../components/code-highlighter/page.mdx) or [`useCode`](../../hooks/use-code/page.mdx) (these use the `SourceEnhancer` function signature)
- **Type documentation**: Passed as `enhancers` to [`abstractCreateTypes`](../../factories/abstract-create-types/page.mdx) (these are standard rehype `PluggableList` entries)
...
The [`loadPrecomputedTypes`](../../pipeline/load-precomputed-types/page.mdx) loader:
