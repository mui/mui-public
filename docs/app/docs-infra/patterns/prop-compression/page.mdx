...
Decompression speed for the large snippet is around 10 ms, but with `highlightAt: 'idle'`, this work done is split into
multiple tasks for each snippet to avoid blocking the main thread.
...
matters just as much. Using the large snippet as an example, here is how each
...
guaranteed to be above the fold on first render — the user sees color
