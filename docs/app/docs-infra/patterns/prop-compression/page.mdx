...
For the `AlertDialog` hero demo, this HAST tree is 16.76 KB: 14.5× larger
than the 1.08 KB plain text. An HTML string would be 4.79 KB, but rendering
it safely often requires shipping extra parsing logic. For example, if we were
to parse this HTML using `rehype-parse` on the client so that we can enhance the code block with interactivity,
...
(27% smaller than plain text HTML, 80% smaller than the highlighted HTML string).
...
payload drops from 0.97 KB to 0.82 KB (24% smaller than plain text HTML and 83%
smaller than the highlighted HTML string), as shown by the "Compressed with Dict" results
...
The highlighted HAST for this snippet serializes to 16.76 KB of JSON, 14.5×
larger than the plain text. An equivalent highlighted HTML string would be
4.79 KB. By compressing the HAST with DEFLATE, the deferred payload drops to
0.97 KB (27% smaller than the plain text HTML). With a content-aware dictionary,
it drops further to 0.82 KB.
...
With [a very large code snippet](https://github.com/mui/mui-public/blob/530ec94f97bfe64ae018a9fd8ff0c326cbd61298/docs/app/bench/docs-infra/components/code-highlighter/snippets/large/snippet.ts), the savings become significant:
...
| Highlight on Init    | 203 KB       | 35 KB         | 0 bytes         | = 238 KB     | 506% |
...
weight of 149 KB versus 406 KB, a 63% reduction. The highlighted payload
itself is 54 KB instead of 203 KB because DEFLATE exploits the repetitive
structure of HAST JSON. Meanwhile, the initial HTML stays at 47 KB of plain
text in both cases, so first paint is unaffected.
...
| 2. RSC payload      | 47 KB      | Hydration (minimal props, no highlighting) |
