...
a separate HTML parser. This is simpler to reason about, has no parser bundle cost, and offers
...
| Highlight on Init    | 193 bytes    | 168 bytes     | 0 byes          | = 361 bytes  | 415% |
...
For a more typical example of a code snippet, we use the example from the `AlertDialog` hero demo.
This is detailed in the [how it works](#how-it-works) section above, but here is a summary of the size comparison:
...
| Highlight on Init    | 4.79 KB      | 0.82 KB       | 0 byes          | = 5.61 KB    | 422% |
...
| Compressed with Dict | 47 KB        | 47 KB         | 35 KB           | = 129 KB     | 274% |
| Highlight on Init    | 203 KB       | 35 KB         | 0 byes          | = 238 KB     | 506% |
