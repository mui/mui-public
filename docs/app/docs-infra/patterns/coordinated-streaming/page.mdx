...
- `useChunks()` streams the _list_ on the client, accumulating snapshots into state.
- `useChunksController()` scopes a group of chunks so the page can tell when they have **all** loaded — via a known count, or a last-chunk flag when the count is unknown.
