...
```ts
// An enum
/** @docs-enum cssVariables Accordion.Panel */
...
The `@docs-enum <kind> <Component>` tag on the export documents the group as a component's table. The kind is `dataAttributes` or `cssVariables`, and the component is named as the entrypoint exports it:

```ts
/** @docs-enum dataAttributes AlertDialog.Popup */
export * as AlertDialogPopupDataAttributes from '../dialog/popup/DialogPopupDataAttributes';
```

The tag belongs on the export, so one module can serve several components, as `DialogPopupDataAttributes` does for both `Dialog.Popup` and `AlertDialog.Popup`. A tag with an unknown kind, or naming a component the entrypoint doesn't export, fails the build. Untagged groups are documented on their own, with a member table.
...
- **Components** (`formatComponentData`) — extracts props, and data attributes and CSS variables from the [constant groups](#constant-groups) tagged for the component.
...
- A [constant group](#constant-groups) → marked as re-export of the tagged component's data attributes or CSS variables
