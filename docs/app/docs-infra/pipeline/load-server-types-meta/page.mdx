...
A group belongs to the component with the longest name its own name starts with, ignoring dots: `AccordionPanelCssVars` belongs to `Accordion.Panel`, and `AlertDialogPopupDataAttributes` to `AlertDialog.Popup` even when its values come from `Dialog`. The values decide what the group holds: `data-` prefixed values are data attributes, `--` prefixed values are CSS variables. Groups that aren't exported from the entrypoint aren't documented.
...
- **Components** (`formatComponentData`) — extracts props, and data attributes and CSS variables from the [constant groups](#constant-groups) the component owns.
...
- A [constant group](#constant-groups) → marked as re-export of its owning component's data attributes or CSS variables
