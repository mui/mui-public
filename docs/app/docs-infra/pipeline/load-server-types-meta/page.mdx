...
With these patterns, `AccordionPanelCssVariables` holds the CSS variables of `Accordion.Panel`. Only the exported name counts, so one module can serve several components: `DialogPopupDataAttributes.ts` published as `AlertDialogPopupDataAttributes` documents `AlertDialog.Popup`. A group whose name matches a pattern but no single exported component fails the build. Groups matching no pattern, and all groups when no patterns are configured, are documented on their own with a member table.
...
Identifies raw type exports that only repeat a component's table and documents them as a link to it:

- `Button.Props` → marked as re-export of Button's props table
- A [constant group](#constant-groups) → marked as re-export of the matched component's data attributes or CSS variables
