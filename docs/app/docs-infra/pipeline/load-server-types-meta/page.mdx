...
- Attaches the data attributes and CSS variables an entrypoint exports to their components
...
- **Folds constant namespaces** — the parser flattens `export * as Name from './module'` into one export per member; when the module holds nothing but literal constants, the members are collapsed back into a single [constant group](#constant-groups) named `Name`
...
A component's data attributes and CSS variables come from **constant groups** its entrypoint publishes: named, documented sets of constants, exported either as an enum or as a namespace of constants.
...
```ts
// AccordionPanelCssVars.ts: named literal constants
/**
 * The accordion panel's height.
 * @type {number}
 */
...
// index.ts
export * as AccordionPanelCssVars from './AccordionPanelCssVars';
```

Both produce identical documentation. The constant form lets application bundlers inline and tree-shake the values, since nothing references a runtime enum object.

Which groups hold a component's table is decided by name patterns, where `*` stands for the component's name with its dots removed. By default, `*DataAttributes` holds data attributes and `*CssVariables` holds CSS variables, so `AccordionPanelCssVariables` holds the CSS variables of `Accordion.Panel`. Configure other patterns with `constantGroupPatterns`, passed to `withDocsInfra` (or directly to `loadServerTypesMeta`), or pass `{}` to attach no tables:

````js
withDocsInfra({
  constantGroupPatterns: { dataAttributes: '*DataAttributes', cssVariables: '*CssVars' },
});
``` Only the exported name counts, so one module can serve several components: `DialogPopupDataAttributes.ts` published as `AlertDialogPopupDataAttributes` documents `AlertDialog.Popup`. A group whose name matches a pattern but no single exported component fails the build. A matched group is documented with its declaration, as a `declare namespace` of constants or an `enum` depending on how it is exported, and a link to the component's table. Groups matching no pattern are documented on their own with a member table.

### 4. Build Type Compatibility Map
...
- **Components** (`formatComponentData`) — extracts props, and data attributes and CSS variables from the [constant groups](#constant-groups) matched to the component.
...
- **Raw types** (`formatRawData`) — formats the type declaration as code. Enums and constant groups are declared as they are exported, with a member table for groups not matched to a component.

Each formatter calls `rewriteTypeStringsDeep` at the end, which applies the type compatibility map from step 4 to normalize all type references. Rewrites use word-boundary matching to prevent partial replacements (e.g., `Dialog` won't match inside `AlertDialog`).
...
Identifies raw type exports that only repeat a component's props and documents them as a link to it, e.g. `Button.Props` → marked as re-export of Button's props table.

### 9. Organize by Export
...
The socket lives in the system temp directory (or the runner's temp directory on GitHub Actions and
Azure Pipelines), in a directory named after a hash of the project directory so that several
checkouts can build at the same time without sharing a socket. Keeping it out of the project matters
because Unix domain socket paths are capped at 104 bytes on macOS and 108 on Linux, which a
project-relative path exceeds once the checkout sits deep enough — in a git worktree, for instance.

Set `MUI_DOCS_INFRA_SOCKET_DIR` to override the directory on a host whose temp directory can't hold a
socket. It is used as given, so it has to be absolute, short enough to stay under that cap, and
distinct per checkout if several run concurrently.
