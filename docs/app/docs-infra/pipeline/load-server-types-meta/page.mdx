...
```ts
// An enum
export enum AccordionPanelCssVariables {
  /**
...
// index.ts
export * as AccordionPanelCssVariables from './AccordionPanelCssVars';
```
...
A group's name tells which component table it holds: the component's name with its dots removed, followed by `DataAttributes` for data attributes or `CssVariables` for CSS variables. So `AccordionPanelCssVariables` holds the CSS variables of `Accordion.Panel`. Only the exported name counts, so one module can serve several components: `DialogPopupDataAttributes.ts` published as `AlertDialogPopupDataAttributes` documents `AlertDialog.Popup`. A group named this way whose component part matches no single exported component fails the build. A matched group is documented with its declaration, as a `declare namespace` of constants or an `enum` depending on how it is exported, and a link to the component's table. Groups named otherwise are documented on their own with a member table.
