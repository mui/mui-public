...
Which groups hold a component's table is configured with `constantGroupPatterns`, passed to `withDocsInfra` (or directly to `loadServerTypesMeta`). In each pattern, `*` stands for the component's name with its dots removed:
...
With these patterns, `AccordionPanelCssVariables` holds the CSS variables of `Accordion.Panel`. Only the exported name counts, so one module can serve several components: `DialogPopupDataAttributes.ts` published as `AlertDialogPopupDataAttributes` documents `AlertDialog.Popup`. A group whose name matches a pattern but no single exported component fails the build. A matched group is documented with its declaration, as a `declare namespace` of constants or an `enum` depending on how it is exported, and a link to the component's table. Groups matching no pattern, and all groups when no patterns are configured, are documented on their own with a member table.
