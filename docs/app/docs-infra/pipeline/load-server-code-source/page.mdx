...
The `loadServerCodeSource` utility reads source files from the filesystem and analyzes their dependencies, extracting imports and resolving relative file paths. It processes JavaScript/TypeScript files to build dependency trees and prepare code for documentation and live demos.
...
> [!IMPORTANT]
> The values in `extraFiles` are file URLs (e.g., `'file:///app/utils.ts'`), not source code. This allows [`loadIsomorphicCodeVariant`](../load-isomorphic-code-variant/page.mdx) to recursively load each dependency file by calling `loadSource` again with the URL.
...
2. **Parse Imports**: Extracts relative imports (starting with `./` or `../`), external dependencies (npm packages), and comments using [`parseImportsAndComments`](../loader-utils/page.mdx#parseimportsandcomments)
3. **Resolve Paths**: For JavaScript/TypeScript files, resolves import paths to absolute file URLs using [`resolveImportResult`](../loader-utils/page.mdx#resolveimportresult) (e.g., `./Button` → `/app/components/Button/index.tsx`)
4. **Process Imports**: Builds the `extraFiles` mapping and optionally rewrites import statements based on `storeAt` mode using [`processRelativeImports`](../loader-utils/page.mdx#processrelativeimports)
5. **Filter Comments**: If `removeCommentsWithPrefix` is set, strips matching comments from the source output
6. **Build Dependencies**: Creates `extraDependencies` array containing file URLs for [`loadIsomorphicCodeVariant`](../load-isomorphic-code-variant/page.mdx) to recursively load
7. **Return Comments**: Returns all parsed comments (by line number) for use by [`sourceEnhancers`](../load-isomorphic-code-variant/page.mdx#enhancing-parsed-code)
...
const result = await loadServerCodeSource('file:///app/components/button/Demo.tsx');
// Result:
// {
...
const result = await loadServerCodeSource('file:///app/components/button/Button.tsx');
// Result:
// {
//   source: "import React from 'react';\nimport { Box } from '@mui/material';",
//   externals: {
...
const result = await loadServerCodeSource('file:///app/components/button/styles.css');
// Result:
// {
...
```ts
const flatLoader = createLoadServerCodeSource({ storeAt: 'flat' });
const canonicalLoader = createLoadServerCodeSource({ storeAt: 'canonical' });
...
- [`createLoadServerCodeSource`](#configuration) - Factory function for customizing load behavior
- [`loadServerCodeMeta`](../load-server-code-meta/page.mdx) - For loading demo metadata from `index.ts` files
- [`parseImportsAndComments`](../loader-utils/page.mdx#parseimportsandcomments) - For parsing import statements and comments
- [`processRelativeImports`](../loader-utils/page.mdx#processrelativeimports) - For processing and rewriting import paths
- [`resolveImportResult`](../loader-utils/page.mdx#resolveimportresult) - For resolving module paths with filesystem
