# Load Server Code Source

[//]: types.ts '<-- Autogenerated By (do not edit the following markdown directly), run: pnpm docs:validate docs-infra/pipeline/load-server-code-source'

## API Reference

### createLoadServerCodeSource

Creates a loadSource function that reads a file and extracts its dependencies.

**Parameters:**

| Parameter | Type                          | Default | Description |
| :-------- | :---------------------------- | :------ | :---------- |
| options?  | `LoadServerCodeSourceOptions` | -       | -           |

**Return Value:**

```tsx
type ReturnValue = LoadSource;
```

### loadServerCodeSource

Default loadServerCodeSource function that reads a file and extracts its dependencies.
This function is used to load source files for demos, resolving their imports and dependencies.
It reads the source file, resolves its imports, and returns the processed source along with any
additional files and dependencies that were found.

**Parameters:**

| Parameter | Type     | Default | Description |
| :-------- | :------- | :------ | :---------- |
| url       | `string` | -       | -           |

**Return Value:**

```tsx
type ReturnValue = Promise<{
  source: string;
  extraFiles?: VariantExtraFiles;
  extraDependencies?: string[];
  externals?: Externals;
  comments?: SourceComments;
}>;
```

## External Types

### LoadSource

```typescript
type LoadSource = (url: string) => Promise;
```
