...
Pass it as the `loadCodeMeta` function — `CodeHighlighter` calls it with the `url` itself, then loads each resolved variant with `loadSource`:
...
function DemoPage() {
  return (
    <CodeHighlighter
      url="file:///app/demos/example/index.ts"
...
More commonly, it is wired once into your `createDemo` factory (via [`abstractCreateDemo`](../../factories/abstract-create-demo/page.mdx)) so every server-loaded demo uses it automatically.
...
2. **Parse Factory Call**: Uses [`parseCreateFactoryCall`](../parse-create-factory-call/page.mdx) to find `createDemo()` or `createDemoWithVariants()` calls
...
If the file contains no `create*` call or no variants, the function returns an empty `Code` object rather than throwing.
...
- [`loadIsomorphicCodeVariant`](../load-isomorphic-code-variant/page.mdx) - For loading individual variant metadata
- [`loadServerCodeSource`](../load-server-code-source/page.mdx) - For loading source code content
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['loadCodeMeta', 'variants', 'demo parsing', 'path resolution'],
  });
