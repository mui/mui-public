...
The loader injects a `precompute` value shaped like the `Code` type from
`@mui/internal-docs-infra/CodeHighlighter/types` (simplified):

```typescript
interface PrecomputedCode {
  [variantName: string]: {
    /** Main file name */
    fileName: string;
    /** Highlighted source — a HAST tree, or its JSON/compressed form per the `output` option */
    source: HastRoot | { hastJson: string } | { hastCompressed: string };
    /** Additional dependencies, keyed by relative path, each with its own `source` */
    extraFiles?: {
      [path: string]: { source: HastRoot | { hastJson: string } | { hastCompressed: string } };
    };
    /** Transform manifest — keys name each transform (e.g. 'JS') */
    transforms?: {
      [transformName: string]: { fileName?: string; hasDelta?: boolean };
    };
  };
}
```
