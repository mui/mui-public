...
- **Type-safe metadata**: Extends [Next.js `Metadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata#the-metadata-object) with `audience` support via inline `@type` annotation
...
- The `@type` annotation provides IDE autocomplete and type checking against Next.js `Metadata`
...
The `Metadata` type **extends Next.js [`Metadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata#the-metadata-object)** with an `audience` field for page categorization. All standard Next.js metadata fields (`title`, `description`, `openGraph`, `robots`, etc.) remain available.

Use the inline `@type` JSDoc annotation for type checking and IDE autocomplete in MDX files:
...
The annotation enables:

- **IDE autocomplete** for all Next.js metadata fields plus `audience`
- **Type checking** to catch invalid values at build time
- **Prettier-stable** formatting (the multi-line format is idempotent)

Available `audience` values:

| Value            | Meaning                                      |
| ---------------- | -------------------------------------------- |
| _(undefined)_    | Public page, intended for all users          |
| `'private'`      | Internal page, excluded from public indexing |
...
> **Note**: Setting `audience` to `'private'` does **not** change the `robots` metadata. To exclude a page from search engine indexing, you must explicitly set `robots: { index: false }` in your `metadata` export.
...
const remarkPlugins = [
  ...
  transformMarkdownMetaLinks, // Handle meta links (See Demo, See Setup, See Types)
...
- [`createSitemap`](../../factories/create-sitemap/page.mdx) - Uses extracted metadata to build searchable sitemaps
