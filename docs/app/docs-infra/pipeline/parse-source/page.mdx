...
`.jsx` is highlighted with the TypeScript JSX grammar. The full extension-to-grammar mapping lives in `grammarMaps.ts` (exported as `extensionMap`); unlisted extensions fall back to plain text.
