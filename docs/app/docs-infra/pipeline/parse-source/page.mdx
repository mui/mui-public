...
// Use the HAST tree for rendering (e.g., with hastToJsx)
```
...
The plain-text path is also exported directly as `parsePlainText(source)` for
callers that want the same un-highlighted HAST shape without going through
language detection.
...
1. **Initialization**: `createParseSource()` creates a Starry Night instance with all grammar definitions (loaded as a code-split chunk). Pass `initialScopes` (possibly `[]`) to create a lean instance that registers grammars on demand via `registerGrammars` — the per-language path used by [`CodeProviderLazy`](../../components/code-provider/page.mdx#per-language-grammar-loading)
...
`di-this` applies to JS/TS family files (`.js`, `.ts`, `.tsx`, `.jsx`, `.mdx`). `di-bt` applies to TypeScript files only (`.ts`, `.tsx`, `.mdx`) — JavaScript is excluded because `string`, `number`, etc. are valid variable names there. The whole family shares one grammar, so this is decided from the file's extension or its explicit `language`, not from the grammar scope. `di-jsx` and `di-jv` only apply to JSX grammars (`.tsx`, `.jsx`, `.mdx`); `di-jv` is added to identifier spans (`pl-smi`, `pl-v`) appearing inside JSX expression braces (`{...}`), including spread arguments, arrow-function parameters, and `pl-c1` member-access property names (e.g. `name` in `{row.name}`). Object property keys inside JSX expressions also receive `di-jv` in addition to `di-op`. `di-ak`, `di-ae`, `di-av` only apply to HTML/JSX/MDX grammars. `di-pu` applies to all grammars and tags `pl-k` operator tokens whose text is purely symbolic (it is not added to word keywords like `const` or `if`). `di-op` applies to JS-family grammars and tags both bare-identifier object keys (e.g. `height` in `{ height: 400 }`) and string keys (e.g. `'aria-label'`); string keys additionally receive `di-ps`. `di-da`, `di-cp`, and `di-cv` only apply to CSS. `di-ht` and `di-jt` are added by `enhanceCodeInline` (not `parseSource`) as wrapper spans around tag brackets and the inner `pl-ent` or `pl-c1` span.
...
The whole JavaScript family — `.js`, `.jsx`, `.ts`, `.tsx` — is highlighted with the TypeScript JSX grammar, so a file and its TypeScript twin are classified identically and JSX in a `.js` file needs no configuration. Type-only styling is still withheld from JavaScript. The full extension-to-grammar mapping lives in `grammarMaps.ts` (exported as `extensionMap`); unlisted extensions fall back to plain text.
...
<TypesParseSource.registerGrammars />

[See Types](./types.md#registerGrammars)
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['starry night', 'syntax highlighting', 'grammars', 'HAST', 'TextMate'],
  });
