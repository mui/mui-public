...
| `di-p`    | Parameter / identifier      | `foo` in `import { foo } from 'bar'`                |
...
`di-ak`, `di-ae`, `di-av` only apply to HTML/JSX/MDX grammars. `di-da`, `di-cp`, `di-cv`, `di-cvar` only apply to CSS. The rest are language-agnostic.
