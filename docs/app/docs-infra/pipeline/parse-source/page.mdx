...
| --------- | --------------------------- | -------------------------------- |
| `di-num`  | Numeric constant            | `42`, `3.14`, `0xFF`, `100px`    |
| `di-bool` | Boolean                     | `true`, `false`                  |
| `di-n`    | Nullish value               | `null`, `undefined`, `""`, `''`  |
| `di-ae`   | Attribute equals            | `=` in `<div className="x">`     |
| `di-av`   | Attribute value             | `"x"` in `className="x"`         |
| `di-da`   | CSS data attribute selector | `data-active` in `[data-active]` |

`di-ae`, `di-av` only apply to HTML/JSX/MDX grammars. `di-da` only applies to CSS. The rest are language-agnostic.
