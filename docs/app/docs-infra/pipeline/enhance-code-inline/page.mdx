...
When the tag-name span also carries `di-jsx` (added by `extendSyntaxTokens` for JSX components), the wrapper is always `di-jt`, regardless of the base class.
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['inline code', 'tag wrapping', 'di-ht', 'di-jt', 'token reclassification'],
  });
