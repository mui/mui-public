...
While a change is pending, `<Pre>` annotates the rendered `<pre>` with
`data-transforming=""` and sets a `--docs-infra-transform-delay` custom
property matching the JS delay. The `pre[data-transforming] .collapse`
rule in
[`syntax.css`](https://github.com/mui/mui-public/blob/master/docs/app/docs-infra/components/code-highlighter/demos/syntax.css)
animates each placeholder from `height: 0` back to
`n * var(--docs-infra-syntax-line-height)` over that same duration, so
the JS commit lands the moment the placeholders have finished
expanding. A subsequent click during the pending window cancels the
previous timer and re-arms with the new target.
