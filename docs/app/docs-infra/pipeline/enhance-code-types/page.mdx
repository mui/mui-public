...
The `enhanceCodeTypes` plugin scans syntax-highlighted code for type names and converts matching spans into anchor links. It supports both inline `<code>` elements and code blocks within `<pre>` elements, and handles dotted chains like `Accordion.Trigger.State`.
...
const enhancers = [[enhanceCodeTypes, { anchorMap }]];
```
