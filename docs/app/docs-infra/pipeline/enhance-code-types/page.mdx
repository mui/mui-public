...
<a href="#justify-content" class="pl-c1">justify-content</a>: <a href="#justify-content:space-between" data-name="justify-content" data-prop="space-between" class="pl-c1">space-between</a>;
```
...
- **`data-exports`** — a JSON array of all exports found in the code block:
  ...
  - `type` — _(optional)_ the type annotation or inferred literal type for non-object `const`/`let`/`var` exports. When a type annotation is present (e.g., `export const x: MyType = ...`), the annotation name is used. Otherwise, simple literal values (strings, numbers, booleans) are captured. Omitted when the type cannot be determined (e.g., function call results).
  - `properties` — _(optional)_ a structured key/value map for `kind: 'object'` exports. CSS class exports use this to store selector literals, for example `{ button: '.button' }`.
