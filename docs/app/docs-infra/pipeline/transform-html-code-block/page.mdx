...
Pass `disableOversizedFocus: true` to make blocks whose focused region exceeds `focusFramesMaxSize`
collapse to nothing (an empty collapsed window) instead of showing a truncated slice. See
[`enhanceCodeEmphasis`](../enhance-code-emphasis/page.mdx#collapsing-to-nothing-with-disableoversizedfocus)
for details.
...
block is hidden until the reader expands it. Unlike `disableOversizedFocus` (which only affects
