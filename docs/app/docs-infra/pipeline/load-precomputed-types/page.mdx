...
> [!NOTE]
> The loader works with any `create*` function that follows the types factory pattern, not just `createTypes`. You can create custom factory functions using [`abstractCreateTypes`](../../factories/abstract-create-types/page.mdx) like `createApiReference()` or `createTypeDocs()` that return documentation components.
...
Configure the webpack/Turbopack loader with these options. See the [Types](#types) for the full `LoaderOptions` definition.
