...
The `transformHtmlCodeInline` plugin transforms plain inline code into beautifully syntax-highlighted code using [Starry Night](https://github.com/wooorm/starry-night). Unlike [`transformHtmlCodeBlock`](../transform-html-code-block/page.mdx), this plugin is optimized for inline code snippets and type expressions.
...
- **Inline marker**: Adds a `data-inline` attribute to processed inline code so CSS can target it
- **Block code skipped**: Code inside `<pre>` is left for [`transformHtmlCodeBlock`](../transform-html-code-block/page.mdx) (opt in with `includePreElements`)
...
| :------------------- | :------ | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `includePreElements` | boolean | `false` | Also process `<code>` elements inside `<pre>`. By default block code is skipped (it's handled by `transformHtmlCodeBlock`). Code inside `<pre>` never receives the `data-inline` attribute. |
...
The language comes from the `language-*` class name; aliases are normalized before mapping to the [supported grammars](../parse-source/page.mdx#supported-languages):
...
2. **Find code elements**: Visits all `<code>` elements in the HAST tree, skipping those inside `<pre>` (unless `includePreElements` is set)
...
5. **Apply prefix**: Temporarily prepends prefix for proper context. Without a prefix, a JS-family snippet that looks like a bare object literal (`{ height: 400 }`) is wrapped in `(...)` so it parses as an expression instead of a block statement
6. **Highlight**: Uses Starry Night to apply syntax highlighting, then extends the tree with the additive `di-*` classes from [`extendSyntaxTokens`](../parse-source/page.mdx#syntax-token-classes)
7. **Remove prefix**: Strips prefix (and any object-literal wrapping) characters from the highlighted output
8. **Replace children**: Updates code element with highlighted nodes
9. **Clean up**: Adds the `data-inline` marker (inline code only) and removes the `data-highlighting-prefix` attribute
...
- Code inside `<pre>` elements (unless `includePreElements` is set)

---

## Comparison with transformHtmlCodeBlock

| Feature             | transformHtmlCodeInline | transformHtmlCodeBlock |
| :------------------ | :---------------------- | :--------------------- |
| **Use case**        | Inline code snippets    | Multi-line code blocks |
| **Line gutters**    | No                      | Yes                    |
| **Frame wrapper**   | No                      | Yes                    |
| **Data attributes** | `data-inline` marker    | Yes (dataPrecompute)   |
...
import { TypesTransformHtmlCodeInline } from './types';

<TypesTransformHtmlCodeInline />
...
- [`enhanceCodeInline`](../enhance-code-inline/page.mdx) - The next stage: wraps tag brackets and reclassifies tokens in the highlighted output
- [`transformHtmlCodeBlock`](../transform-html-code-block/page.mdx) - For multi-line code blocks with line gutters
- [`parseSource`](../parse-source/page.mdx) - Underlying syntax highlighting function
- [`formatProperties`](../load-server-types-meta/page.mdx#6-format-types) - Uses this for type highlighting
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['inline highlighting', 'type expressions', 'highlighting prefix', 'data-inline'],
  });
