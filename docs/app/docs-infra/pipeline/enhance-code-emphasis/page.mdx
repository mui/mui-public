...
| `data-frame-focus-target` | marker | Present on the authored focus frame when configured padding adds surrounding context. It distinguishes the focus target from its padding frames, including when the target frame is also `highlighted`.                                                                                                                          |
| `data-frame-indent`       | number | Only on `highlighted`, `highlighted-unfocused`, `focus`, and `focus-unfocused` frames, and only when `createEnhanceCodeEmphasis({ emitFrameIndent: true })` is used. The shared indent level of the region lines (min leading spaces ÷ 2).                                                                                       |
...
| `focusFramesMaxSize`  | number                   | `12`         | Maximum total lines in the focus area (padding + region). When the region fits, remaining budget is split floor/ceil between padding-top and padding-bottom. When the region exceeds this limit, a focused window is taken from the start of the region, and the remaining overflow lines are marked as unfocused (unless `oversizedFocus: 'hide'` is set).                                                                                                   |
...
When configured padding adds surrounding context, the authored focus frame receives a `data-frame-focus-target` marker. This lets styles distinguish the focus target from its padding frames, even when the target also has `data-frame-type="highlighted"`:
