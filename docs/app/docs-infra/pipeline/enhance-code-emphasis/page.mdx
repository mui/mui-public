...
This adds `data-hl-text="Heading 1"` to the line element, which can be used by CSS or JavaScript to highlight only that specific text within the line.
...
```ts
{
  ...
  properties: {
    ...
    dataHlText: 'specific text'             // Optional: specific text to highlight
  },
...
| `data-hl-text`        | string | Specific text to highlight within the line (for `@highlight-text`)                                 |
...
For text highlighting with `@highlight-text`:

- `data-hl` is set
- `data-hl-position="single"` is set
- `data-hl-text` contains the text to highlight
