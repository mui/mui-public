...
/* Fade overlay at the bottom of truncated code blocks. Uses `::after` with a
   linear-gradient and `transform` so the animation is GPU-accelerated; the
   pre's computed overflow clips the translated pseudo-element. The explicit
   `> code > .frame` child path bounds the `:has()` scan to a single, fixed
   depth instead of walking the whole subtree. `padding-bottom: 0` removes
   the pre's bottom padding while truncated so the fade sits flush against
   the last visible line; the padding is restored once expanded. */
...
pre.codeBlock:has(> code > .frame[data-frame-truncated='visible'])::after {
  content: '';
  position: absolute;
  bottom: 0;
  left: 0;
...
.expanded pre.codeBlock:has(> code > .frame[data-frame-truncated='visible']) {
  padding-bottom: var(--code-padding-bottom);
}

/* Slide the fade overlay out of view when the block is expanded. */
.expanded pre.codeBlock:has(> code > .frame[data-frame-truncated='visible'])::after {
