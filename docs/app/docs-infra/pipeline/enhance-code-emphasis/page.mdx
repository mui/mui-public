...
When the focused region exceeds `focusFramesMaxSize`, no visible-window frame is produced. The block stays `collapsible`, but `focusedLines` is `0`, so the collapsed state is empty and expanding reveals the entire source. This applies to oversized `@highlight` and `@focus` regions, and to the auto-focus-from-line-1 case when a snippet with no emphasis comments exceeds `focusFramesMaxSize`.
