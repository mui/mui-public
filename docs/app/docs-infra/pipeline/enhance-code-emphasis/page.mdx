...
| `focusFramesMaxSize`    | number  | —       | Maximum total lines in the focus area (padding + region). When the region fits, remaining budget is split floor/ceil between padding-top and padding-bottom. When the region exceeds this limit, a focused window is taken from the start of the region, and the remaining overflow lines are marked as unfocused (unless `disableOversizedFocus` is set).                                                                                                    |
| `disableOversizedFocus` | boolean | `false` | When `true`, a focused region larger than `focusFramesMaxSize` is not truncated into a visible window. Instead no focus frame is produced: the block collapses to nothing (`focusedLines === 0`, still `collapsible`), so the collapsed state is empty and expanding reveals the whole source. Applies to oversized `@highlight` / `@focus` regions and the auto-focus-from-line-1 case (no comments).                                                        |
...
### Collapsing to Nothing with `disableOversizedFocus`

By default, a focused region larger than `focusFramesMaxSize` is truncated: the first `focusFramesMaxSize` lines stay visible and the overflow is marked unfocused. For some layouts a partial slice is worse than no focus at all — you'd rather show the whole snippet, collapsed away entirely until the reader expands it.

Set `disableOversizedFocus: true` to opt into that behavior:
