...
The `<pre>` element exposes both counts as `data-total-lines` and `data-focused-lines` so CSS and consumers can react to the empty collapsed window (e.g. render an "expand" affordance when `data-focused-lines="0"`).
