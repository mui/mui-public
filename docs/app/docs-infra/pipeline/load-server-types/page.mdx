...
> **Note**: `highlightedVariantData` contains `EnhancedTypesMeta` with HAST fields (`type`, `shortType`, `detailedType`, `default`), while `allTypes` contains the original `TypesMeta` with plain text fields (`typeText`, `defaultText`).
