...
The [`IsomorphicImports`](#isomorphicimports) shape mirrors what `parseImportsAndComments` produces after name flattening, so resolvers can inspect each import's `url`, named bindings (`names`), type-only flag (`includeTypeDefs`), and source `positions` — see the [Types](#types) section for the full definition.
...
The returned function satisfies the [`LoadSource`](#return-value) contract:
...
`Externals` maps each module specifier to its import bindings (`name`, `type: 'named' | 'default' | 'namespace'`, `isType`) — see [`Externals`](../../components/code-highlighter#externals) in the CodeHighlighter types.
