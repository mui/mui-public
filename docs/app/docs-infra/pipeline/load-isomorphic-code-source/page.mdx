...
The [`IsomorphicImports`](./types.md#IsomorphicImports) shape mirrors what `parseImportsAndComments` produces after name flattening, so resolvers can inspect each import's `url`, named bindings (`names`), type-only flag (`includeTypeDefs`), and source `positions` — see the [Types](#types) section for the full definition.
...
`storeAt` decides the file-tree shape every downstream consumer sees:

- **`'flat'`** — best for display and in-browser editing: each variant reads as a
  single folder and imports are rewritten to match. This is what the demo
  pipeline uses by default.
- **`'canonical'`** — preserves the real directory structure (including
  `index.*` resolution), so consumers that rebuild a file tree —
  [`flattenCodeVariant`](../load-isomorphic-code-variant/page.mdx#flattening-for-file-systems)
  and the
  [StackBlitz / CodeSandbox exports](../../hooks/use-demo/page.mdx#open-in-stackblitz-or-codesandbox)
  built on it — reproduce the project as it exists on disk.
- **`'import'`** — keys mirror the authored import specifiers (with extensions
  added but without `index.*` resolution); useful when the displayed file list
  should match the import statements.

## Return Value

The returned function satisfies the [`LoadSource`](./types.md#LoadSource) contract:

```ts
const result = await loadSource('https://example.com/Demo.tsx');
// {
//   source: string — processed source code (may have rewritten imports)
//   extraFiles?: Record<string, string> — import path → URL for recursive loading
//   extraDependencies?: string[] — URLs for recursive loading
//   externals?: Externals — external npm package dependencies
//   comments?: Record<number, string[]> — 1-indexed line → comments (for sourceEnhancers)
// }
```

`Externals` maps each module specifier to its import bindings (`name`, `type: 'named' | 'default' | 'namespace'`, `isType`) — see [`Externals`](../../components/code-highlighter/types.md#Externals) in the CodeHighlighter types.
...
// result.comments: { 1: ['@internal This is an implementation detail'] }
// Keys are 1-indexed line numbers in the stripped source — the comment attaches
// to the line that follows it (here, the "User-visible comment" line).
...
// result.comments: { 1: ['@highlight Important line'] }
// The comment attaches to `const x = 1;` — line 1 of the stripped source.
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['loadSource', 'fetchSource', 'resolveImports', 'storeAt', 'import parsing'],
  });
