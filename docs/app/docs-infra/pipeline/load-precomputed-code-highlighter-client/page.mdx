...
The client file doesn't declare variants itself — the loader discovers them from the paired `index.ts` (e.g. a `createDemoWithVariants` call) and collects and merges externals across all of them:
...
// Externals are collected from every variant defined in ./index.ts
export const MultiVariantDemoClient = createDemoClient(import.meta.url);
```
...
Each entry maps a module path to the actual imported values resolved at build time:
...
```typescript
// ✓ demo-1/client.ts — use separate files instead
'use client';
...
If the paired `index.ts` cannot be read, or contains no `create*` call or variants, the loader logs a warning and leaves the client file unprocessed (no externals are injected). If a variant file itself fails to load, the loader fails the build with an error naming the variant.

### Server-Only Imports

If any variant imports a server-only module (`fs`, `node:*`, or the `server-only` package), the loader leaves the client file untouched rather than inlining server code into the client bundle. The downstream factory then sees no `precompute.externals` and skips wrapping with a provider.
...
- Dependencies resolve through the normal bundler graph, so they are deduped with the rest of the app
...
import { TypesLoadPrecomputedCodeHighlighterClient } from './types';

<TypesLoadPrecomputedCodeHighlighterClient />
...
- [loadPrecomputedCodeHighlighter](../load-precomputed-code-highlighter/page.mdx) - Main demo loader for syntax highlighting
- [withDocsInfra](../with-docs-infra/page.mdx) - Next.js plugin that configures this loader automatically
- [abstractCreateDemoClient](../../factories/abstract-create-demo-client/page.mdx) - Client factory implementation
- [CodeHighlighter](../../components/code-highlighter/page.mdx) - Code highlighting component system

export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['externals', 'demo client', 'webpack loader', 'live editing', 'createDemoClient'],
  });
