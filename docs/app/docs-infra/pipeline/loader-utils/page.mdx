...
- **`deriveRelativeUrls`** - Derives the original import path for rewritten `extraFiles` entries
...
Returns a map of `extraFileKey -> relativeUrl` for entries whose key, when resolved against the source file's URL, no longer points at the file itself. This typically happens in `'flat'` mode (and selectively in `'import'` mode) where keys are rewritten relative to the source file's directory.

The returned `relativeUrl` is normalized to start with `./` or `../`, so consumers can rebuild the original file URL via `new URL(relativeUrl, sourceFileUrl)`. Entries where the key already resolves to the file URL (typically `'canonical'` mode) are omitted.
...
const { extraFiles } = processRelativeImports(
  source,
  importResult.relative,
  'flat',
  true,
  resolvedPathsMap,
);
// extraFiles: { './styles.css': 'file:///src/styles.css' }

const relativeUrls = deriveRelativeUrls(sourceFileUrl, extraFiles);
// relativeUrls: { './styles.css': '../styles.css' }

// Reconstruct the original URL:
new URL(relativeUrls['./styles.css'], sourceFileUrl).href;
// → 'file:///src/styles.css'
```
