...
- **`getImportExtensionPriority`** - Picks the extension priority for one file's imports
...
const result = parseImportsAndComments(source, 'file:///src/current/file.tsx');
console.log(result);
// {
//   relative: {
...
```typescript
const result = parseImportsAndComments(source, fileUrl, {
...
// - comments: Record<lineNumber, commentContent[]> (lineNumber is 1-indexed)
```
...
Picks the extension priority for the imports written in one file. Pass the result as `extensions` so an extensionless import resolves against the sibling matching the importing file's language.

```ts
import { getImportExtensionPriority } from '@mui/internal-docs-infra/pipeline/loaderUtils';

getImportExtensionPriority('Button.tsx'); // ['.ts', '.tsx', '.js', '.jsx', …]
getImportExtensionPriority('Button.js'); // ['.js', '.jsx', '.ts', '.tsx', …]
```

This matters for projects that ship both languages of a file. Where `data.js` sits next to `data.ts`, `import { rows } from './data'` should reach the JavaScript copy from `Button.js` and the TypeScript copy from `Button.tsx`. Without the override both resolve to `data.ts`, so the JavaScript demo displays a TypeScript file.

`loadServerCodeSource` applies this automatically, per file, as it walks the import graph. Anything with `.js`, `.jsx`, `.mjs`, or `.cjs` counts as a JavaScript importer; everything else keeps the TypeScript-first order.
...
const { relative } = parseImportsAndComments(source, fileUrl);
...
async function processCodeImports(source: string, fileUrl: string) {
  ...
  const { relative, externals } = parseImportsAndComments(source, fileUrl);
...
import { TypesLoaderUtils } from './types';

<TypesLoaderUtils />

[See Types](./types.md#LoaderUtils)
