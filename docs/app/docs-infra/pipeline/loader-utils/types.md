...
segment is not a whole route group. Uses the same whole-segment rule as [`isRouteGroup`](#isroutegroup), so
...
filters whole segments with [`isRouteGroup`](#isroutegroup), so a folder that merely contains parentheses
