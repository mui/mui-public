...
- **Skip when present**: Files that already contain a valid `@focus` directive are left untouched. Highlight directives remain independent, so a file containing only `@highlight` markers still receives an auto-generated focus region.
...
Enable the `wrapReturn` option to wrap bare `return <X />` statements in parentheses so the focus comment can be placed inside:
