...
| `performance`                | `object`   | `{}`                           | Performance logging options (see below)                                                                                      |
| `deferCodeParsing`           | `string`   | `'gzip'`                       | Output format: `'gzip'`, `'json'`, or `'none'`                                                                               |
| `removeCommentsWithPrefix`   | `string[]` | `undefined`                    | Comment prefixes to strip from source (e.g., `['@internal']`)                                                                |
| `notableCommentsPrefix`      | `string[]` | `undefined`                    | Comment prefixes to collect for [`sourceEnhancers`](../../hooks/use-code/page.mdx#source-enhancers) (e.g., `['@highlight']`) |
| `typesIndexFileName`         | `string`   | `'page.mdx'`                   | Index file name for types metadata sync                                                                                      |
| `errorIfTypesIndexOutOfDate` | `boolean`  | `Boolean(process.env.CI)`      | Throw error if types index is out of date                                                                                    |
