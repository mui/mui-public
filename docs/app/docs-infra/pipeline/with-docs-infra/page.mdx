...
export default withDocsInfra({
  ...
  // Cap concurrent source transforms per webpack worker. Defaults to
  // `os.cpus().length` when `transformTypescriptToJavascript` is enabled —
  // override to a smaller number if production builds approach the heap limit.
