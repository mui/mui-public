...
```tsx
import { CodeProvider } from '@mui/internal-docs-infra/CodeProvider';
import type { LoadCodeMeta, LoadSource } from '@mui/internal-docs-infra/CodeHighlighter/types';
...
Both providers also accept a `sourceEnhancers` prop to post-process the parsed
HAST for every block in their subtree — see
[Source Enhancers](../../hooks/use-code/page.mdx#source-enhancers) in the
`useCode` docs for how enhancers work.
...
Each parse request accepts an optional `AbortSignal`. When a newer keystroke supersedes an older one, the textarea editor aborts the older signal so the worker's response is dropped on arrival rather than replacing fresher highlighted output. Pre-aborted signals reject synchronously without any cross-thread traffic.

### Wiring

The worker integration is fully automatic — no opt-in is required. A consumer that wraps an editable code block in `CodeProvider` will see worker-backed highlighting as long as `Worker` is defined and the editable's host (typically [`useCode`](../../hooks/use-code/page.mdx)) provides a `setSource` callback. Until the worker is ready, or when it is unavailable, the editor displays escaped plain text.
...
consumer. They are the variant loader, fallback loader, transform-delta computer,
client-side transform applier (both pull `jsondiffpatch`), and the
`@highlight`/`@focus` emphasis enhancer (~13&nbsp;KB). The textarea editor and
source-state helpers are separate lazy modules. Loaded only when a block needs
them, these boundaries keep a read-only block's `useCode` chunk smaller.

- **`CodeProvider` (eager)** statically bundles the code-processing functions, so
  they resolve instantly with no fetch. The textarea editor remains lazy so a
  read-only page never loads `react-simple-code-editor`.
- **`CodeProviderLazy`** loads them via dynamic `import()` on demand, keeping them
  ...
  speculatively preloads the right chunk on first render (when it detects it will
  fetch variant data, compute transforms client-side, or host an editable
  controller), in parallel with the content, so the fetch is usually in
  flight — or already resolved — before the content needs it.
...
post-swap transforms for the loaders/delta computer, and effects once a block
becomes editable for the editor and source-state helpers. The synchronous
...
- **[CodeHighlighter](../code-highlighter/page.mdx)**: The component that consumes the functions this provider supplies
- **[useCode](../../hooks/use-code/page.mdx)**: Hook for building `Content` components — includes the Source Enhancers guide
- **[CodeControllerContext](../code-controller-context/page.mdx)**: Controlled code state for the editable blocks whose engine loading is described above
- **[ChunkProvider](../chunk-provider/page.mdx)**: The same provider layering pattern applied to the coordinated-lazy system
