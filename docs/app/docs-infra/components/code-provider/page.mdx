...
The heaviest functions - the variant loader, the fallback loader, the
transform-delta computer (which pulls `jsondiffpatch`), and the live-editing
engine (the `contentEditable` setup + keystroke/caret/paste handling used by
editable blocks) - are exposed through the **same accessor interface** by two
...
post-swap transforms for the loaders/delta computer, and (for the editing engine)
a layout effect once a block becomes editable. The synchronous
