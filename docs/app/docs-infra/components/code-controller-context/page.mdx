...
The focus trap addresses this by introducing a deliberate "armed → engaged" two-step interaction for keyboard users, while leaving mouse users completely unaffected.
...
- When the wrapper receives keyboard focus (matched via `:focus-visible`), it sets `data-editable-armed=""` on itself and reveals an overlay child instructing the user to press <kbd>Enter</kbd> to start editing.
...
The wrapper and overlay ship with no styling — they expose stable class names and a data attribute so consumers can style them entirely from CSS:

| Selector                                      | Description                                                                                  |
| :-------------------------------------------- | :------------------------------------------------------------------------------------------- |
| `.editable-code-wrapper`                      | The outer focusable wrapper. Use `position: relative` to anchor the overlay.                 |
| `.editable-code-wrapper[data-editable-armed]` | Present only while the wrapper holds keyboard focus and the prompt is shown.                 |
| `.editable-code-wrapper:focus-visible`        | Style the wrapper's keyboard focus ring.                                                     |
| `.editable-code-overlay`                      | The "Press Enter to start editing" prompt. Always rendered — hide it from CSS based on the wrapper's `data-editable-armed` attribute. |
...
/* The overlay is always mounted (so it can animate in/out); hide it by
   default and reveal it only while the wrapper is armed. */
...
.code :global(.editable-code-wrapper)[data-editable-armed] :global(.editable-code-overlay) {
  visibility: visible;
}
