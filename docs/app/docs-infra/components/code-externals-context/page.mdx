...
const context = {
  externals: {
    react: React, // Default or namespace import
...
Only bare module specifiers (packages) appear in `externals` — relative imports (`./helper`) are loaded as the demo's own files instead.
...
function SafeExternalDemo() {
  ...
  const { '@mui/material': MUI = {} } = externalsContext.externals;
  const { Button } = MUI;
...
function ConditionalDemo() {
  ...
  const { Button, Typography } = externals['@mui/material'] || {};

  return (
    <div>
      {Button && <Button>Button Available</Button>}
      {Typography && <Typography>Typography Available</Typography>}
      {!Button && !Typography && <div>No MUI components available</div>}
    </div>
  );
}
```
...
[See Types](./types.md#CodeExternalsContext)
...
- **[`abstractCreateDemoClient`](../../factories/abstract-create-demo-client/page.mdx)**: Creates providers that supply this context
- **[`loadPrecomputedCodeHighlighterClient`](../../pipeline/load-precomputed-code-highlighter-client/page.mdx)**: Loader that injects externals
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['externals', 'live demo', 'react-runner', 'scope', 'dependencies'],
  });
