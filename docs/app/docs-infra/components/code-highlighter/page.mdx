...
> [!TIP]
> **For Documentation Tooling**: If you're building tooling to render these demos in a web interface, you can use a remark plugin to automatically remove the "See Demo" links and horizontal rule separators (`---`) from the rendered output, since the interactive demos provide their own navigation. See the [`transformMarkdownDemoLinks`](../../pipeline/transform-markdown-demo-links/page.mdx) plugin documentation for implementation details.
...
- `sourceEnhancers` (SourceEnhancers): Functions to enhance parsed HAST sources (see [Source Enhancers](../../hooks/use-code/page.mdx#source-enhancers))
...
// Enhancer function to modify parsed HAST
type SourceEnhancer = (
  root: HastRoot,
...
type SourceEnhancers = Array<SourceEnhancer>;
```
