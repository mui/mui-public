...
- **Multiple variants** for showing different implementation styles
...
> [!NOTE]
> Before you can use `createDemo()` in your demos, you must first create this factory function in your repo (see the [`abstractCreateDemo`](../../factories/abstract-create-demo/page.mdx#implementation) docs for details).
...
You implement `createDemo` within your app using [`abstractCreateDemo`](../../factories/abstract-create-demo/page.mdx#implementation) to define the structure and behavior of your demos.
...
> [!NOTE]
> For detailed information about the webpack loader implementation, configuration, and advanced usage, see the [Precompute Loader Documentation](../../pipeline/load-precomputed-code-highlighter/page.mdx).
...
import { DemoCodeHighlighterCodeTransformedAddition } from './demos/code-transformed-addition';

<DemoCodeHighlighterCodeTransformedAddition.Title />

Transformers can also _add_ lines. Marking each new line with a
`@expanding` comment — or bracketing a contiguous run with paired
`@expanding-start` / `@expanding-end` comments — in the returned comments
map tells the runtime to animate those lines' heights during the swap,
mirroring how transformer-removed lines collapse. Toggle the switch to
see a randomly generated API key constant slide in at the top of the
snippet:

<DemoCodeHighlighterCodeTransformedAddition />

[See Demo](./demos/code-transformed-addition/)

---

import { DemoCodeHighlighterCodeHighlightInit } from './demos/code-highlight-init';

<DemoCodeHighlighterCodeHighlightInit.Title />

For code examples that you know will be above the fold, you can highlight them immediately with `highlightAfter="init"` (the default is `'idle'`).
To keep the initial payload small, only the initially visible region is highlighted — the focused region when one is marked, otherwise the first 120-line frame.
...
**Writing a `ContentLoading` component.** Call `useCodeFallback(props)` to read the
fallback `source` (and, for multi-file or multi-variant fallbacks, `extraSource` /
`extraVariants`), render it with `hastToJsx`, and wrap each file in semantic markup —
a `<dl>` with the filename in `<dt>` and the code in `<dd>` — so crawlers and
`transformHtmlCodeBlock` can read every file from the initial HTML. `useCodeFallback`
also hoists the fallback up to the client, where its text is the DEFLATE dictionary
used to decompress the `hastCompressed` source; a `ContentLoading` component that
doesn't call it will throw. See the demo above for the full pattern.
...
### Payload and Timing Tuning

Three lesser-known props tune how much work and how many bytes a block ships:

- **`deferParsing`** (`'none' | 'json' | 'gzip'`, default `'gzip'`) — controls how
  the precomputed AST crosses to the client in production RSC payloads: as a
  live object (`'none'`), JSON-stringified (`'json'`), or DEFLATE-compressed
  with the [dictionary scheme](../../pipeline/hast-utils/page.mdx#compression)
  (`'gzip'`). The deferred forms keep the AST out of memory until the block is
  enhanced.
- **`enhanceAfter`** (`'init' | 'stream' | 'hydration' | 'idle'`, default
  `'idle'`) — when the code display is enhanced with interactivity; the
  companion to `highlightAfter` for the enhancement step.
- **`fallbackCollapsed`** — paints only the collapsed window in the
  `ContentLoading` fallback and defers each file's full fallback into the
  compressed payload. This shrinks the initial HTML of a collapsed block to its
  on-screen lines — but removes the hidden lines from the server-rendered
  markup, so it is **only** appropriate for content that will not be crawled
  (authenticated or internal pages). See the
  [prop-compression pattern](../../patterns/prop-compression/page.mdx) for the
  full trade-off.
...
> [!TIP]
> **For Documentation Tooling**: If you're building tooling to render these demos in a web interface, you can use a remark plugin to automatically remove the "See Demo" links and horizontal rule separators (`---`) from the rendered output, since the interactive demos provide their own navigation. See the [`transformMarkdownMetaLinks`](../../pipeline/transform-markdown-meta-links/page.mdx) plugin documentation for implementation details.

## Benchmarking

For performance benchmarks of the `CodeHighlighter` component, see the [Benchmarking Code Highlighter](./bench/page.mdx) page.
...
Use the factory pattern for optimal performance:
...
3. **Handle errors gracefully** - Loading and parsing errors propagate to the nearest React error boundary; wrap demos in one to display meaningful error messages
4. **Optimize highlight timing** - `highlightAfter` defaults to `'idle'`; reserve `'init'` for above-the-fold code blocks
...
- Use the demo factory pattern for build-time optimization
- Keep the default `highlightAfter: 'idle'` for non-critical code instead of `'init'`
...
- **[useCode](../../hooks/use-code/page.mdx)**: Build the `Content` component — file navigation, transforms, copy, and source enhancers
- **[useDemo](../../hooks/use-demo/page.mdx)**: Build demo `Content` components that pair the rendered component with its code
- **[CodeProvider](../code-provider/page.mdx)**: Client-side loading, highlighting, and the lazy-chunk runtime
- **[CodeControllerContext](../code-controller-context/page.mdx)**: Controlled code state for editable blocks and live demos
- **[abstractCreateDemo](../../factories/abstract-create-demo/page.mdx)**: Implement the `createDemo` factory for precomputed demos
...
export const metadata =
  /** @type {import('@mui/internal-docs-infra/createSitemap/types').NextMetadata} */ ({
    keywords: ['syntax highlighting', 'code block', 'demo', 'precompute', 'variants', 'fallback'],
  });
