...
interface SitemapPage {
  title?: string; // Resolved title (reflects any user override from the index)
  slug: string;
  path: string;
  description?: string;
  keywords?: string[];
...
The sitemap data is designed for use with Orama search. For a ready-made React hook that handles index creation, querying, and result formatting, see [`useSearch`](../../hooks/use-search/page.mdx).
...
- **Title** from the first H1 heading (or a user-overridden title if the list item was edited in the index. See [`syncPageIndex` title overrides](../../pipeline/sync-page-index/page.mdx#title-overrides))
...
Additional sitemap metadata is managed through the index files maintained by [`syncPageIndex`](../../pipeline/sync-page-index/page.mdx). For example, in your index `page.mdx`:
...
- **Tags**: `[New]` is a lifecycle label that appears in the sitemap as `page.tags`. See [`syncPageIndex` tags](../../pipeline/sync-page-index/page.mdx#tags).
- **Title overrides**: "Input" overrides the page's actual H1 ("Text Field") for display in sitemaps and search. See [`syncPageIndex` title overrides](../../pipeline/sync-page-index/page.mdx#title-overrides).
...
- [`syncPageIndex`](../sync-page-index/page.mdx) - Maintain index pages with tags, title overrides, and ordering
