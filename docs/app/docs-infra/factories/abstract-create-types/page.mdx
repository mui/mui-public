...
## Additional CodeHighlighter Types
...
> [!TIP]
> The default `'visible'` works well for most documentation sites. The visual difference is minimal — most users don't expand detailed types immediately — and the server rendering savings can be substantial. Use `'init'` only if you need fully-highlighted type blocks in the initial server-rendered HTML.
