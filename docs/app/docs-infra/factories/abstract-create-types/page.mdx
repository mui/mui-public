...
**Important**: The custom component must be included in both `components` and `inlineComponents` maps. The `components` map is used for code blocks (inside `<pre>`), while `inlineComponents` is used for inline code (inside `<code>`).
