...
export function filterCommits(commits, filterConfig) {
  ...
  return commits.filter((commit) => {
    ...
    if (filterConfig.excludeAuthors && filterConfig.excludeAuthors.length > 0) {
      const authorLogin = commit.author?.login;
      if (authorLogin) {
        // Check if author matches any exclusion pattern
        for (const pattern of filterConfig.excludeAuthors) {
    ...
    if (filterConfig.excludeLabels && filterConfig.excludeLabels.length > 0) {
      // Check if commit has any excluded labels
      for (const excludedLabel of filterConfig.excludeLabels) {
