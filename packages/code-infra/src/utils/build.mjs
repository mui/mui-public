...
export function getVersionEnvVariables(pkgVersion) {
   ...
   */
  const res = {
    MUI_VERSION: JSON.stringify(pkgVersion),
    MUI_MAJOR_VERSION: JSON.stringify(major),
    MUI_MINOR_VERSION: JSON.stringify(minor),
    MUI_PATCH_VERSION: JSON.stringify(patch),
    MUI_PRERELEASE: prerelease ? JSON.stringify(prerelease) : 'undefined',
  };
  return res;
}
