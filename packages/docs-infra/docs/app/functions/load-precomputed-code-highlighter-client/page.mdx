...
│           └── advanced-demo/
│               ├── index.ts      # ← Main demo
│               ├── client.ts     # ← Client version processed here
│               ├── TypeScript.ts
│               └── JavaScript.js
```
...
Discovers related variant files by reading the corresponding main demo file (`index.ts` in the same directory) to extract variant information, name, and slug.
  ...
  import.meta.url,
  {
    precompute: {
      externals: {
        ...
        '@mui/system': [
          { name: 'styled', type: 'named', isType: false }
        ]
      }
    }
  }
...
Each external module contains an array of imports with this structure:

```typescript
interface ExternalImport {
  name: string;        // Import name ('default', 'Button', etc.)
  type: string;        // Import type ('default', 'named', 'namespace')
  isType: boolean;     // Whether this is a type-only import (filtered out)
}

interface Externals {
  [modulePath: string]: ExternalImport[];
}
```
```
