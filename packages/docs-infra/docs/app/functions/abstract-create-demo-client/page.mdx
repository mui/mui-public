...
- **[`loadPrecomputedCodeHighlighter`](../load-precomputed-code-highlighter/page.mdx)**: For optimized loading with precomputed externals
...
> **Note:** `createDemoClient` must be called in a file with `'use client';` because it creates a client component. `createDemo` should **not** be called in a client file, as it may interfere with server-side loading and parsing.
...
- **[`useRunner`](../../hooks/use-runner/page.mdx)**: Hook for executing code with externals
