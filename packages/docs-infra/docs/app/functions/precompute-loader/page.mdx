...
The loader processes demo files that use the `createDemo` factory pattern with `precompute: true`, automatically resolving and processing all code variants at build time rather than runtime.

> **Note**: The loader works with any `create*` function, not just `createDemo`. You could have `app/components/my-component/snippets/example/index.ts` with `createSnippet()`, or any other factory function that follows the same pattern with `precompute: true`.
...
For Next.js projects using Webpack (instead of Turbopack), add the loader to your `next.config.mjs`:
  ...
  webpack: (config, { buildId, dev, isServer, defaultLoaders, webpack }) => {
    config.module.rules.push({
      ...
      use: [
        ...
        '@mui/internal-docs-infra/loadPrecomputedCodeHighlighter'
      ]
    });
...
export const BasicDemo = createDemo(
  import.meta.url,
  {
    Default
  },
  {
    name: 'Basic Example',
    slug: 'basic',
    precompute: true,  // ← Enables precomputation
  },
);
```
...
```typescript
import { createDemo } from '../createDemo';
import TypeScript from './Component.tsx';
import JavaScript from './Component.jsx';

export const MultiVariantDemo = createDemo(
  import.meta.url,
  {
    TypeScript,
    JavaScript,
...
export const ComponentSnippet = createSnippet(
  import.meta.url,
  {
    Default,
  },
  {
    ...
    slug: 'component',
    precompute: true,  // ← Works with any create* function
  },
);
```
...
### 4. Replace Precompute Value

Replaces `precompute: true` in your source with the processed data.
...
The loader replaces `precompute: true` with a data structure containing:
...
interface PrecomputedData {
  [variantName: string]: {
    ...
    extraFiles: {              // Additional dependencies
      [path: string]: HastNode[];
    };
    transforms: {              // Language variants
