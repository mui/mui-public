...
const nextConfig = {
  turbopack: {
    rules: {
      './app/**/types.ts': {
        as: '*.ts',
        loaders: ['@mui/internal-docs-infra/loadPrecomputedTypesMeta'],
      },
    },
  },
  webpack: (config, { buildId, dev, isServer, defaultLoaders, webpack }) => {
    config.module.rules.push({
      test: /\/types\.ts$/,
      use: [defaultLoaders.babel, '@mui/internal-docs-infra/loadPrecomputedTypesMeta'],
    });
...
export const TypesComponent = createTypesMeta(
  import.meta.url,
  Component,
  {
    precompute: {
...
type AnyType = ArrayNode | ComponentNode | EnumNode | ExternalTypeNode | 
               FunctionNode | IntersectionNode | IntrinsicNode | LiteralNode | 
               ObjectNode | TupleNode | TypeParameterNode | UnionNode;
...
declare class IntrinsicNode implements TypeNode {
  ...
  typeName: TypeName | undefined;
  intrinsic: 'string' | 'number' | 'boolean' | 'bigint' | 'null' | 'undefined' | 'void' | 'any' | 'unknown' | 'function';
}
