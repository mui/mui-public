...
describe('resolveChunkRender', () => {
  ...
  // (still loading) and, if a server full loader exists, loads the full in the
  ...
  describe('have-initial branch (isInitial: the initial paint is already in hand)', () => {
    ...
    it('ignores the initial loader when the initial is already in hand', () => {
      // We already have the initial paint, so a server initial is irrelevant -
      // with no full loader we just render the content (loading).
      expect(
        resolveChunkRender({
          ...base,
          isInitial: true,
  ...
  // No full content AND no initial paint in hand: render a quick initial on the
  // server first, else go straight to the server full loader, else let the client
  // attempt it (loading via a ChunkProvider source).
  describe('load-initial branch (no initial paint in hand)', () => {
    it('renders the server initial loader', () => {
      expect(resolveChunkRender({ ...base, hasServerInitial: true })).toEqual({
        mode: 'server-initial',
        loading: true,
      });
    });
    ...
    it('attempts the initial data on the client when no server provider exists at all', () => {
  ...
  it('prefers rendering an initial over loading the full when neither is in hand', () => {
