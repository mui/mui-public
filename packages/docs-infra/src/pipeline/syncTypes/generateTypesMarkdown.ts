...
export async function generateTypesMarkdown(
  ...
  // The leading `<--` points at the `types.ts` reference the comment carries, which
  // editors surface as a link to the source these types were generated from.
  ...
  const headerChunk = markdownChunk([
    md.heading(1, name),
    md.commentRef('types.ts', commentText),
