...
describe('metadataToMarkdown', () => {
  ...
  it('should rewrite absolute image URLs to relative public paths when path is provided', () => {
    const data: PagesMetadata = {
      title: 'Components',
      pages: [
        {
          ...
          description: 'A button component.',
          image: {
            url: '/docs-infra/components/button/screenshot.png',
            alt: 'Button screenshot',
          },
        },
      ],
    };
    ...
    const result = metadataToMarkdown(data, {
      path: 'app/docs-infra/components/page.mdx',
    });

    expect(result).toContain(
      '![Button screenshot](../../../public/docs-infra/components/button/screenshot.png)',
    );
  });

  it('should not rewrite non-absolute image URLs when path is provided', () => {
    const data: PagesMetadata = {
      title: 'Components',
      pages: [
        {
    ...
    expect(result).toContain('![Button image](https://example.com/button.png)');
  });

  it('should not rewrite absolute image URLs when path is not provided', () => {
    const data: PagesMetadata = {
      title: 'Components',
      pages: [
        {
    ...
    expect(result).toContain('![Button screenshot](/docs-infra/components/button/screenshot.png)');
  });
  ...
  it('should wrap content in component when indexWrapperComponent is provided', () => {
    ...
    expect(result).toContain('</PagesIndex>');
    // Ensure wrapper is around the content (metadata marker inside, closing tag after)
    const openingTagIndex = result.indexOf('<PagesIndex>');
    const closingTagIndex = result.indexOf('</PagesIndex>');
    const metadataMarkerIndex = result.indexOf('remainder of the file can be modified');
    expect(openingTagIndex).toBeLessThan(metadataMarkerIndex);
    expect(metadataMarkerIndex).toBeLessThan(closingTagIndex);
  });
...
describe('metadataToMarkdownAst', () => {
  it('should convert page metadata to markdown AST', () => {
    ...
    // Check list - new format: Title - ([Outline](#slug), [Contents](./path))
    ...
    expect(paragraph.children[0].value).toBe('Button'); // Title text
    expect(paragraph.children[1].value).toBe(' - ('); // Separator
    expect(paragraph.children[2].type).toBe('link');
    expect(paragraph.children[2].url).toBe('#button'); // Outline link
    expect(paragraph.children[2].children[0].value).toBe('Outline');
    expect(paragraph.children[3].value).toBe(', '); // Comma separator
    expect(paragraph.children[4].type).toBe('link');
    expect(paragraph.children[4].url).toBe('./button/page.mdx'); // Contents link
    expect(paragraph.children[4].children[0].value).toBe('Contents');
    expect(paragraph.children[5].value).toBe(')'); // Closing paren
  ...
  it('should rewrite absolute image URLs to relative public paths in AST when path is provided', () => {
    const data: PagesMetadata = {
      title: 'Components',
      pages: [
        {
    ...
    expect(imageNode).toBeDefined();
    expect(imageNode.children[0].url).toBe(
      '../../../public/docs-infra/components/button/screenshot.png',
    );
  });
...
`;
  ...
  it('should convert relative public image URLs back to absolute paths', async () => {
    const markdown = `# Components
...
`;
    ...
    expect(result?.pages[0]?.image?.url).toBe('/docs-infra/components/button/screenshot.png');
  });
...
- Button - ([Outline](#button), [Contents](./button/page.mdx))
- Nested Component - ([Outline](#nested-component), [Contents](./nested/component/page.mdx))
- Simple - ([Outline](#simple), [Contents](./simple.mdx))
...
- myFunction - ([Outline](#myfunction), [Contents](./my-function/page.mdx))
- My Component - ([Outline](#my-component), [Contents](./my-component/page.mdx))
- Product\u00a0Name - ([Outline](#productname), [Contents](./product-name/page.mdx))
...
- myFunction - ([Outline](#my-function), [Contents](./my-function/page.mdx))
- anotherFunction - ([Outline](#another-function), [Contents](./another-function/page.mdx))
...
- Example Component - ([Outline](#example-component), [Contents](./example-component/page.mdx))
...
- Base UI - ([Outline](#base-ui), [Contents](./base-ui/page.mdx))
...
- useErrors - ([Outline](#use-errors), [Contents](./use-errors/page.mdx))
...
`;
    ...
    expect(result?.pageMetadata).toEqual({ robots: { index: false } });
  });

  it('should preserve pageMetadata when metadata marker is inside wrapper', async () => {
    const markdown = `# Components
...
`;
    ...
    expect(result).toBeDefined();
    expect(result?.pageMetadata).toEqual({ robots: { index: true } });
  });
...
`;
  ...
  // TODO: Remove this describe block once all index files have been migrated to the new format.
  describe('old format migration', () => {
    it('should parse old format and produce correct metadata', async () => {
      const oldFormatMarkdown = `# Components
...
`;
      ...
      const result = await markdownToMetadata(oldFormatMarkdown);
      ...
      expect(result?.pages[0].slug).toBe('button');
      expect(result?.pages[0].title).toBe('Button');
      expect(result?.pages[0].path).toBe('./button/page.mdx');

      expect(result?.pages[1].slug).toBe('checkbox');
      expect(result?.pages[1].title).toBe('Checkbox');
      expect(result?.pages[1].tags).toEqual(['New']);
      expect(result?.pages[1].path).toBe('./checkbox/page.mdx');
    });

    it('should re-serialize old format into new format', async () => {
      const oldFormatMarkdown = `# Components
...
`;
      ...
      const parsed = await markdownToMetadata(oldFormatMarkdown);
      expect(parsed).toBeDefined();

      const reserialized = metadataToMarkdown(parsed!);

      // Should output new format
      expect(reserialized).toContain(
        '- Button [Hot] - ([Outline](#button), [Contents](./button/page.mdx))',
      );
      // Should NOT contain old format
      expect(reserialized).not.toContain('[Full Docs]');
      expect(reserialized).not.toContain('- [Button](#button)');
    });
  });

  describe('private and index fields', () => {
    it('should parse Private status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Private, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBe('private');
      expect(result?.pages[0].index).toBeUndefined();
    });

    it('should parse Index status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Index, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBe('private');
      expect(result?.pages[0].index).toBe(true);
    });

    it('should parse Public Index status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Public Index, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBeUndefined();
      expect(result?.pages[0].index).toBe(true);
    });

    it('should not set private or index for entries without status label', async () => {
      const markdown = `# Components
...
`;
    ...
    it('should parse Introductory status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Introductory, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBe('introductory');
      expect(result?.pages[0].index).toBeUndefined();
    });

    it('should parse Introductory Index status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Introductory Index, [Outline](#button), [Contents](./button/page.mdx))
...
`;
    ...
    it('should parse Intermediate status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Intermediate, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBe('intermediate');
      expect(result?.pages[0].index).toBeUndefined();
    });

    it('should parse Intermediate Index status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Intermediate Index, [Outline](#button), [Contents](./button/page.mdx))
...
`;
    ...
    it('should parse Advanced status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Advanced, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      expect(result?.pages[0].audience).toBe('advanced');
      expect(result?.pages[0].index).toBeUndefined();
    });

    it('should parse Advanced Index status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Advanced Index, [Outline](#button), [Contents](./button/page.mdx))
...
`;
    ...
    it('should throw on invalid status label', async () => {
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Button - (Bogus, [Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      await expect(markdownToMetadata(markdown)).rejects.toThrow('Unknown status label "Bogus"');
    });
  });

  it('should not match markers inside fenced code blocks', async () => {
    const markdown = `# Transform Markdown Metadata
...
export const metadata = {
  keywords: ['remark', 'plugin'],
};
`;

    const result = await markdownToMetadata(markdown);

    // Should not crash trying to parse code block content as metadata
    // The raw regex should skip over markers inside fenced code blocks
    expect(result).toBeDefined();
    expect(result?.pageMetadata).toBeUndefined();
  });
});
...
`;
  ...
  describe('tags support', () => {
    it('should serialize tags in list items', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      expect(result).toContain(
        '- Button [New] [Hot] - ([Outline](#button), [Contents](./button/page.mdx))',
      );
      expect(result).toContain(
        '- Checkbox [Beta] - ([Outline](#checkbox), [Contents](./checkbox/page.mdx))',
      );
    });
...
`;
    ...
    it('should serialize tags in AST format', () => {
      ...
      // Should have: text (title), text (' [New]'), text (' [Hot]'), text (' - ('), link (outline), text (', '), link (contents), text (')')
      expect(paragraph.children.length).toBe(8);
      expect(paragraph.children[0].type).toBe('text'); // Title text
      expect(paragraph.children[0].value).toBe('Button');
      ...
      expect(paragraph.children[4].type).toBe('link'); // Outline link
      expect(paragraph.children[4].url).toBe('#button');
      expect(paragraph.children[5].value).toBe(', ');
      expect(paragraph.children[6].type).toBe('link'); // Contents link
      expect(paragraph.children[6].url).toBe('./button/page.mdx');
      expect(paragraph.children[7].value).toBe(')');
    });
...
- Button [New] [Hot] [Beta] - ([Outline](#button), [Contents](./button/page.mdx))
...
`;
    ...
    it('should serialize single-link entries without detail sections', () => {
      ...
      expect(result).toContain('- Forms - ([Outline](#forms), [Contents](./forms/page.mdx))');
      expect(result).toContain(
        '- TypeScript - ([Outline](#typescript), [Contents](./typescript/page.mdx))',
      );
...
`;
      ...
      it('should serialize exports with parameters for hooks/functions', () => {
        const data: PagesMetadata = {
          title: 'Hooks',
          pages: [
            {
              slug: 'use-dialog',
              path: './use-dialog/page.mdx',
              title: 'useDialog',
              description: 'A hook to manage dialog state.',
              exports: {
                useDialog: {
                  parameters: ['defaultOpen', 'modal', 'onOpenChange', 'open'],
                },
              },
            },
          ],
        ...
        expect(result).toContain('- Exports:');
        expect(result).toContain('  - useDialog');
        expect(result).toContain('    - Parameters: defaultOpen, modal, onOpenChange, open');
      });

      it('should parse exports with parameters from markdown', async () => {
        const markdown = `# Hooks

[//]: # 'This file is autogenerated, but the following list can be modified.'

- [useDialog](#usedialog) - [Full Docs](./use-dialog/page.mdx)

[//]: # 'This file is autogenerated, DO NOT EDIT AFTER THIS LINE'

## useDialog

A hook to manage dialog state.
...
`;
      ...
      it('should preserve parameters through round-trip conversion', async () => {
        const original: PagesMetadata = {
          title: 'Hooks',
          pages: [
            {
      ...
      it('should serialize exports with returns for hooks/functions', () => {
        const data: PagesMetadata = {
          title: 'Hooks',
          pages: [
            {
              ...
              exports: {
                useDialog: {
                  parameters: ['defaultOpen', 'modal', 'onOpenChange', 'open'],
                  returns: ['open', 'setOpen', 'getRootProps'],
                },
              },
            },
          ],
        ...
        expect(result).toContain('  - useDialog');
        expect(result).toContain('    - Parameters: defaultOpen, modal, onOpenChange, open');
        expect(result).toContain('    - Returns: open, setOpen, getRootProps');
      });

      it('should serialize exports with array returns', () => {
        const data: PagesMetadata = {
          title: 'Hooks',
          pages: [
            {
              slug: 'use-toggle',
              path: './use-toggle/page.mdx',
              title: 'useToggle',
              description: 'A hook to manage toggle state.',
              exports: {
                useToggle: {
        ...
        expect(result).toContain('    - Returns: value, toggle, setValue');
      });

      it('should parse exports with returns from markdown', async () => {
        const markdown = `# Hooks
...
`;
      ...
      it('should parse exports with array returns from markdown', async () => {
        const markdown = `# Hooks

[//]: # 'This file is autogenerated, but the following list can be modified.'

- [useToggle](#usetoggle) - [Full Docs](./use-toggle/page.mdx)

[//]: # 'This file is autogenerated, DO NOT EDIT AFTER THIS LINE'

## useToggle

A hook to manage toggle state.
...
`;
      ...
      it('should preserve returns through round-trip conversion', async () => {
        const original: PagesMetadata = {
          title: 'Hooks',
          pages: [
            {
      ...
      it('should escape underscores in parameters to avoid markdown emphasis', () => {
        const data: PagesMetadata = {
          title: 'Functions',
          pages: [
            {
              slug: 'format-type',
              path: './format-type/page.mdx',
              title: 'formatType',
              description: 'Formats a TypeScript type.',
              exports: {
                formatType: {
                  parameters: ['_options', 'exportNames', '_internal'],
                },
              },
            },
          ],
        ...
        // Underscores should be escaped in markdown
        expect(result).toContain('    - Parameters: \\_options, exportNames, \\_internal');
      });

      it('should unescape underscores when parsing parameters', async () => {
        const markdown = `# Functions

[//]: # 'This file is autogenerated, but the following list can be modified.'

- [formatType](#formattype) - [Full Docs](./format-type/page.mdx)

[//]: # 'This file is autogenerated, DO NOT EDIT AFTER THIS LINE'

## formatType

Formats a TypeScript type.
...
[Read more](./format-type/page.mdx)
`;

        const result = await markdownToMetadata(markdown);

        expect(result).not.toBeNull();
        // Underscores should be unescaped when parsing
        expect(result?.pages[0].exports).toEqual({
          formatType: {
            parameters: ['_options', 'exportNames', '_internal'],
          },
      ...
      it('should preserve underscored parameters through round-trip conversion', async () => {
        const original: PagesMetadata = {
          title: 'Functions',
          pages: [
            {
      ...
      it('should serialize object parameters wrapped in ( )', () => {
        const data: PagesMetadata = {
          title: 'Functions',
          pages: [
            {
              slug: 'default-html-template',
              path: './default-html-template/page.mdx',
              title: 'defaultHtmlTemplate',
              description: 'Creates a default HTML template.',
              exports: {
                defaultHtmlTemplate: {
      ...
      it('should parse object parameters wrapped in ( ) as a single entry', async () => {
        const markdown = `# Functions

[//]: # 'This file is autogenerated, but the following list can be modified.'

- [defaultHtmlTemplate](#defaulthtmltemplate) - [Full Docs](./default-html-template/page.mdx)

[//]: # 'This file is autogenerated, DO NOT EDIT AFTER THIS LINE'

## defaultHtmlTemplate

Creates a default HTML template.
...
[Read more](./default-html-template/page.mdx)
`;
      ...
      it('should preserve object parameters through round-trip conversion', async () => {
        const original: PagesMetadata = {
          title: 'Functions',
          pages: [
            {
  ...
  describe('types support', () => {
    it('should serialize types as a comma-separated list', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
            ...
            title: 'Checkbox',
            description: 'A checkbox component.',
            types: ['CheckboxIndicatorState', 'CheckboxRootState'],
          },
        ],
      };
      ...
      expect(result).toContain('- Types: CheckboxIndicatorState, CheckboxRootState');
    });

    it('should parse types from markdown', async () => {
      const markdown = `# Components
...
`;
      ...
      expect(result).not.toBeNull();
      expect(result?.pages[0].types).toEqual(['CheckboxIndicatorState', 'CheckboxRootState']);
    });

    it('should preserve types through round-trip conversion', async () => {
      const original: PagesMetadata = {
        title: 'Components',
        pages: [
          {
            slug: 'accordion',
            path: './accordion/page.mdx',
            title: 'Accordion',
            description: 'An accordion component.',
            types: ['AccordionItemState', 'AccordionRootState', 'AccordionValue'],
          },
        ],
      };
    ...
    it('should handle types alongside exports and parts', async () => {
      const original: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      expect(parsed?.pages[0].parts).toEqual(original.pages[0].parts);
      expect(parsed?.pages[0].exports).toEqual(original.pages[0].exports);
      expect(parsed?.pages[0].types).toEqual(original.pages[0].types);
    });
        ...
        pages: [
          {
            ...
            title: 'Button',
            description: 'A button component.',
            types: ['Button_State', '_InternalType'],
          },
        ],
      };
  ...
  describe('audience and index serialization', () => {
    it('should serialize Private status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Index status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Public Index status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Introductory status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Introductory Index status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Intermediate status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Intermediate Index status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Advanced status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should serialize Advanced Index status label', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should not add status label for pages without audience or index', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should round-trip audience and index fields', async () => {
      const original: PagesMetadata = {
        title: 'Components',
        pages: [
          {
          ...
          {
            slug: 'select',
            path: './select/page.mdx',
            title: 'Select',
            description: 'A select component.',
          },
          {
            slug: 'intro-guide',
            path: './intro-guide/page.mdx',
            title: 'Intro Guide',
          ...
          {
            slug: 'intro-index',
            path: './intro-index/page.mdx',
            title: 'Intro Index',
            description: 'An introductory index.',
            audience: 'introductory',
            index: true,
          },
          {
            slug: 'mid-guide',
            path: './mid-guide/page.mdx',
            title: 'Mid Guide',
            description: 'An intermediate guide.',
            audience: 'intermediate',
          },
          {
            slug: 'mid-index',
            path: './mid-index/page.mdx',
            title: 'Mid Index',
            description: 'An intermediate index.',
            audience: 'intermediate',
            index: true,
          },
          {
            slug: 'adv-guide',
            path: './adv-guide/page.mdx',
            title: 'Adv Guide',
          ...
          {
            slug: 'adv-index',
            path: './adv-index/page.mdx',
            title: 'Adv Index',
            description: 'An advanced index.',
            audience: 'advanced',
            index: true,
          },
        ],
      ...
      // Private (no index)
      expect(parsed?.pages[0].audience).toBe('private');
      expect(parsed?.pages[0].index).toBeUndefined();

      // Private + index
      expect(parsed?.pages[1].audience).toBe('private');
      expect(parsed?.pages[1].index).toBe(true);

      // Public + index (audience is undefined for public pages)
      expect(parsed?.pages[2].audience).toBeUndefined();
      expect(parsed?.pages[2].index).toBe(true);

      // Public (audience is undefined, no index)
      expect(parsed?.pages[3].audience).toBeUndefined();
      expect(parsed?.pages[3].index).toBeUndefined();

      // Introductory (no index)
      expect(parsed?.pages[4].audience).toBe('introductory');
      expect(parsed?.pages[4].index).toBeUndefined();

      // Introductory + index
      expect(parsed?.pages[5].audience).toBe('introductory');
      expect(parsed?.pages[5].index).toBe(true);

      // Intermediate (no index)
      expect(parsed?.pages[6].audience).toBe('intermediate');
      expect(parsed?.pages[6].index).toBeUndefined();

      // Intermediate + index
      expect(parsed?.pages[7].audience).toBe('intermediate');
      expect(parsed?.pages[7].index).toBe(true);

      // Advanced (no index)
      expect(parsed?.pages[8].audience).toBe('advanced');
      expect(parsed?.pages[8].index).toBeUndefined();

      // Advanced + index
      expect(parsed?.pages[9].audience).toBe('advanced');
      expect(parsed?.pages[9].index).toBe(true);
    });
  });

  describe('displayTitle (title override)', () => {
    it('should use displayTitle in list item when set', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
            ...
            path: './button/page.mdx',
            title: 'Button',
            displayTitle: 'Custom Button Name',
            description: 'A button component.',
          },
        ],
      };
      ...
      // List should show displayTitle
      expect(result).toContain(
        '- Custom Button Name - ([Outline](#button), [Contents](./button/page.mdx))',
      );
      // Heading should show the actual title
      expect(result).toContain('## Button');
    });

    it('should use title in list item when displayTitle is not set', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
    ...
    it('should detect displayTitle when list title differs from heading title', async () => {
      // Simulate a user renaming "Button" to "My Button" in the list
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- My Button - ([Outline](#button), [Contents](./button/page.mdx))
...
`;
      ...
      const parsed = await markdownToMetadata(markdown);
      expect(parsed?.pages[0].title).toBe('Button'); // heading title
      expect(parsed?.pages[0].displayTitle).toBe('My Button'); // user's override
      expect(parsed?.pages[0].slug).toBe('button');
    });

    it('should not set displayTitle when list title matches heading title', async () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      expect(parsed?.pages[0].title).toBe('Button');
      expect(parsed?.pages[0].displayTitle).toBeUndefined();
    });

    it('should round-trip displayTitle through serialize and parse', async () => {
      const original: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      // First page has displayTitle
      expect(parsed?.pages[0].title).toBe('Button');
      expect(parsed?.pages[0].displayTitle).toBe('Action Button');

      // Second page has no displayTitle
      expect(parsed?.pages[1].title).toBe('Checkbox');
      expect(parsed?.pages[1].displayTitle).toBeUndefined();
    });

    it('should use Read more link to match when slug derivation fails', async () => {
      // Slug "numberfield" can't be derived from "Number Field" (which gives "number-field")
      const markdown = `# Components

[//]: # 'This section is autogenerated, but the following list order, title, and [Tag]s can be modified, but nothing within the parentheses.'

- Custom Number Input - ([Outline](#numberfield), [Contents](./numberfield/page.mdx))
...
[Read more](./numberfield/page.mdx)
...
`;
      ...
      const parsed = await markdownToMetadata(markdown);
      // The Read more link path corrects the slug match
      expect(parsed?.pages[0].slug).toBe('numberfield');
      expect(parsed?.pages[0].title).toBe('Number Field');
      expect(parsed?.pages[0].displayTitle).toBe('Custom Number Input');
      expect(parsed?.pages[0].description).toBe('A number field component.');
    });

    it('should use displayTitle for single-link entries', () => {
      const data: PagesMetadata = {
        title: 'Resources',
        pages: [
          {
      ...
      expect(result).toContain('- [Official Docs](https://example.com/docs)');
    });

    it('should work with displayTitle and tags', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      expect(result).toContain(
        '- Action Button [New] - ([Outline](#button), [Contents](./button/page.mdx))',
      );
      expect(result).toContain('## Button');
    });

    it('should work with displayTitle and status labels', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      expect(result).toContain(
        '- Action Button - (Private, [Outline](#button), [Contents](./button/page.mdx))',
      );
    });

    it('should use displayTitle in AST output for list items', () => {
      const data: PagesMetadata = {
        title: 'Components',
        pages: [
          {
      ...
      // Find the list node in the AST
      const listNode = ast.children.find((child) => child.type === 'list') as any;
      expect(listNode).toBeDefined();

      // The first list item's paragraph should contain the displayTitle text
      const firstItem = listNode.children[0];
      const paragraphNode = firstItem.children[0];
      const textNode = paragraphNode.children[0];
      expect(textNode.value).toBe('Custom Button');

      // The heading section should use the actual title
      const headingNode = ast.children.find(
        (child) => child.type === 'heading' && (child as any).depth === 2,
      ) as any;
      expect(headingNode).toBeDefined();
      const headingText = headingNode.children[0].value;
      expect(headingText).toBe('Button');
    });
  });
});
