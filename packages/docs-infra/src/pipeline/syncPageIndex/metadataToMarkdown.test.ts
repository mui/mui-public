...
`;
  ...
  describe('content-keyed memoization', () => {
    it('returns isolated clones so a caller mutation cannot affect later calls', async () => {
      const markdown = `# Components
...
`;
      ...
      const first = await markdownToMetadata(markdown);
      ...
      const second = await markdownToMetadata(markdown);
      expect(second?.title).toBe('Components');
      expect(second?.pages).toHaveLength(1);
      expect(second?.pages[0]?.slug).toBe('button');
      expect(second).not.toBe(first);
    });

    it('caches null results for markdown without a title', async () => {
      const markdown = 'Just a paragraph with no heading.\n';

      expect(await markdownToMetadata(markdown)).toBeNull();
      expect(await markdownToMetadata(markdown)).toBeNull();
    });
  });
});
