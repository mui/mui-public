...
describe('a heading of only ungrouped links survives on a synthetic group id', () => {
  ...
  it('leaves detailsSectionTitle unset when no page renders a detail section', async () => {
    // The index is grouped (synthetic section) but every page is an external link, so the
    // renderer writes no "## Details" wrapper. The pre-populated cache metadata must not claim a
    // detailsSectionTitle the rendered file lacks, or the warm cache diverges from a fresh parse.
    const existing = metadataToMarkdown(
      {
        title: 'React',
        sections: [{ group: '#external-links', title: 'External Links' }],
    ...
    const { metadata } = await mergeMetadataPages(existing, {
      title: 'React',
      pages: [{ slug: 'llms', path: '/llms.txt', title: 'llms.txt' }],
    });

    expect(metadata.sections).toEqual([{ group: '#external-links', title: 'External Links' }]);
    expect(metadata.detailsSectionTitle).toBeUndefined();

    // A fresh parse of the rendered file agrees (no wrapper → no title).
    const fresh = await markdownToMetadata(
      metadataToMarkdown(metadata, { path: 'src/app/react/page.mdx' }),
    );
    expect(fresh?.detailsSectionTitle).toBeUndefined();
  });
});

// The "Automatically sorted alphabetically" marker is not mutually exclusive with route-group
// grouping: pages are sorted A-Z globally, then clustered into their sections with stable order,
// so the net effect is alphabetical *within* each section rather than one flat A-Z list.
describe('alphabetical sort applies within each route-group section', () => {
  const ALPHABETICAL_MARKER =
  ...
  it('groups by route group and sorts pages A-Z inside each section', async () => {
    // A flat, alphabetically-sorted index whose pages live in route groups.
    const existing = metadataToMarkdown(
      {
        title: 'React',
        pages: [
          {
            slug: 'zebra',
            path: './(components)/zebra/page.mdx',
            title: 'Zebra',
            description: 'Z.',
          },
          {
            slug: 'alpha',
            path: './(components)/alpha/page.mdx',
            title: 'Alpha',
            description: 'A.',
          },
        ],
      },
      { path: 'src/app/react/page.mdx', editableMarker: ALPHABETICAL_MARKER },
    );

    const { metadata } = await mergeMetadataPages(existing, {
      title: 'React',
      pages: [
        { slug: 'zebra', path: './(components)/zebra/page.mdx', title: 'Zebra', description: 'Z.' },
        { slug: 'alpha', path: './(components)/alpha/page.mdx', title: 'Alpha', description: 'A.' },
        { slug: 'yak', path: './(utils)/yak/page.mdx', title: 'Yak', description: 'Y.' },
        { slug: 'bee', path: './(utils)/bee/page.mdx', title: 'Bee', description: 'B.' },
      ],
    });

    // Sections seeded from the route groups, and pages ordered A-Z within each (not globally).
    expect(metadata.sections?.map((section) => section.group)).toEqual(['(components)', '(utils)']);
    expect(metadata.pages.map((page) => page.slug)).toEqual(['alpha', 'zebra', 'bee', 'yak']);
  });
});
