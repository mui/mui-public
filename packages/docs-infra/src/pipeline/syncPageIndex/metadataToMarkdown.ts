...
export function metadataToMarkdownAst(
  ...
  const metadataCode = `export const metadata = ${typeAnnotation} (${serializeJsObject(metadataObj)});`;
...
export function metadataToMarkdown(
  ...
  if (pageMetadata && Object.keys(pageMetadata).length > 0) {
    lines.push(`export const metadata = ${TYPE_ANNOTATION} (${serializeJsObject(pageMetadata)});`);
  } else {
    lines.push(
      `export const metadata = ${TYPE_ANNOTATION} (${serializeJsObject({ robots: { index: false }, other: { audience: 'private' } })});`,
    );
  }
  lines.push('');
