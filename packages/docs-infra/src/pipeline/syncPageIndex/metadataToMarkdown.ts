...
export async function markdownToMetadata(markdown: string): Promise<PagesMetadata | null> {
  ...
  const metadataExportMatch =
    markdown.match(
      /\[\/\/\]: # 'The above section is autogenerated, but the remainder of the file can be modified\.'\s*\n\s*\n\s*export\s+const\s+metadata\s*=\s*(\{[\s\S]*?\n\})/,
    ) ??
    // TODO: Remove this old marker match once all index files have been migrated to the new format.
    markdown.match(
      /\[\/\/\]: # 'This (?:section|file) is autogenerated, but the following metadata can be modified\.'\s*\n\s*\n\s*export\s+const\s+metadata\s*=\s*(\{[\s\S]*?\n\})/,
    );
