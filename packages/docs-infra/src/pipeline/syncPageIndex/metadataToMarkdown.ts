...
import type { Heading, Paragraph, Image, Link, Root, Code, Nodes } from 'mdast';
...
/**
 * Marker text opening the editable page list.
 */
const DEFAULT_EDITABLE_MARKER =
...
/**
 * Marker text closing the autogenerated section.
 */
...
export function metadataToMarkdownAst(
  ...
  children.push(comment(editableMarker ?? DEFAULT_EDITABLE_MARKER) as any);
  ...
  children.push(comment(METADATA_MARKER) as any);
...
export function metadataToMarkdown(
  ...
  // Add editable section marker
  lines.push(comment(editableMarker ?? DEFAULT_EDITABLE_MARKER).value);
  lines.push('');
  ...
  const doNotEditMarker = quotedPath
    ? `This section is autogenerated, DO NOT EDIT AFTER THIS LINE, run: pnpm docs:validate ${quotedPath}`
    : 'This section is autogenerated, DO NOT EDIT AFTER THIS LINE';
  lines.push(comment(doNotEditMarker).value);
  lines.push('');
  ...
  for (const page of pages) {
    ...
    // Add embeddings as a comment if available
    if (page.embeddings && page.embeddings.length > 0) {
      lines.push(comment(`Embeddings: ${JSON.stringify(page.embeddings)}`).value);
      lines.push('');
    }
  ...
  // Add metadata marker (inside wrapper component if provided)
  lines.push(comment(METADATA_MARKER).value);
  lines.push('');
...
/**
 * Reads the text out of a comment node, or null when the node is not a comment.
 *
 * `.mdx` files carry comments as expression comments and `.md` files as HTML
 * comments. This runs on a plain remark tree with no MDX extension, so an
 * expression comment arrives as a paragraph holding its literal text.
 */
function readCommentMarker(node: Nodes): string | null {
  // TODO: Remove once every index file has been regenerated away from the
  // `[//]: # 'text'` link-definition idiom this package used to emit.
  ...
  if (node.type === 'paragraph' && node.children.length === 1) {
    ...
    if (child.type === 'text') {
      const match = child.value.match(/^\{\/\*([\s\S]*)\*\/\}$/);
      return match ? match[1].trim() : null;
    }
  }
...
async function parseMarkdownToMetadata(markdown: string): Promise<PagesMetadata | null> {
  ...
  visit(tree, (node, index, parent) => {
    // Track sections based on the comment markers separating them
    const markerText = readCommentMarker(node);
    if (markerText !== null) {
      if (
        markerText.includes('following list can be modified') ||
        markerText.includes('following list order')
      ) {
        currentSection = 'editable';
        return;
      }
      if (markerText.includes('DO NOT EDIT AFTER THIS LINE')) {
        currentSection = 'details';
        return;
      }
      if (
        markerText.includes('remainder of the file can be modified') ||
        // TODO: Remove this old marker check once all index files have been migrated to the new format.
        markerText.includes('following metadata can be modified')
      ) {
        currentSection = 'metadata';
        return;
      }
      // Parse embeddings from comment
      if (currentPage && markerText.includes('Embeddings:')) {
        const embeddingsText = markerText.replace('Embeddings:', '').trim();
        try {
  ...
  const metadataExportMatch =
    markdownWithoutCodeBlocks.match(
      /(?:\{\/\*|\[\/\/\]: # ')\s*The above section is autogenerated, but the remainder of the file can be modified\.\s*(?:\*\/\}|')[\s\S]*?export\s+const\s+metadata\s*=\s*(?:\/\*\*[^*]*\*\/\s*\()?\s*(\{[\s\S]*\n\s*\})\)?\s*;?/,
    ) ??
    // TODO: Remove this old marker match once all index files have been migrated to the new format.
    markdownWithoutCodeBlocks.match(
      /(?:\{\/\*|\[\/\/\]: # ')\s*This (?:section|file) is autogenerated, but the following metadata can be modified\.\s*(?:\*\/\}|')[\s\S]*?export\s+const\s+metadata\s*=\s*(?:\/\*\*[^*]*\*\/\s*\()?\s*(\{[\s\S]*\n\s*\})\)?\s*;?/,
    );
