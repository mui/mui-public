...
export interface PageMetadata extends ExtractedMetadata {
  ...
  /** Whether this page is private (robots.index: false in its metadata export) */
  private?: boolean;
...
/**
 * Serializes a JS object to a JavaScript-style object literal string.
 */
function serializeJsObject(obj: Record<string, unknown>): string {
  return serializeJsValue(obj, 0);
}

/**
 * Gets the status label for a page based on its private/index flags.
 * - No label: private=false, index=false (public, not an index)
 * - "Private": private=true, index=false
 * - "Index": private=true, index=true
 * - "Public Index": private=false, index=true
 */
function getStatusLabel(page: PageMetadata): string | undefined {
  const isPrivate = page.private ?? false;
  const isIndex = page.index ?? false;

  if (isPrivate && isIndex) {
    return 'Index';
  }
  if (isPrivate && !isIndex) {
    return 'Private';
...
/**
 * Parses a status label string back into private/index boolean flags.
 */
function parseStatusLabel(label: string): { private: boolean; index: boolean } {
  switch (label.trim()) {
    case 'Private':
      return { private: true, index: false };
    case 'Index':
      return { private: true, index: true };
    case 'Public Index':
      return { private: false, index: true };
    default:
      return { private: false, index: false };
  }
}
...
export function metadataToMarkdownAst(
  ...
  const metadataObj =
    pageMetadata && Object.keys(pageMetadata).length > 0
      ? pageMetadata
      : { robots: { index: false } };
  const metadataCode = `export const metadata = ${serializeJsObject(metadataObj)};`;
...
export function metadataToMarkdown(
  ...
  if (pageMetadata && Object.keys(pageMetadata).length > 0) {
    lines.push(`export const metadata = ${serializeJsObject(pageMetadata)};`);
  } else {
    lines.push(`export const metadata = ${serializeJsObject({ robots: { index: false } })};`);
  }
  lines.push('');
...
export async function markdownToMetadata(markdown: string): Promise<PagesMetadata | null> {
  ...
  const metadataExportMatch =
    markdown.match(
      /\[\/\/\]: # 'The above section is autogenerated, but the remainder of the file can be modified\.'[\s\S]*?export\s+const\s+metadata\s*=\s*(\{[\s\S]*?\n\})/,
    ) ??
    // TODO: Remove this old marker match once all index files have been migrated to the new format.
    markdown.match(
      /\[\/\/\]: # 'This (?:section|file) is autogenerated, but the following metadata can be modified\.'[\s\S]*?export\s+const\s+metadata\s*=\s*(\{[\s\S]*?\n\})/,
    );
