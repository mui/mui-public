...
export function metadataToMarkdownAst(
  ...
  for (const page of pages) {
    ...
    if (hasKeywords || hasSections || hasParts || hasExports || hasTypes) {
      ...
      const detailsPath = page.path.endsWith('/page.mdx')
        ? page.path.substring(0, page.path.length - '/page.mdx'.length)
        : page.path;
      children.push({
        type: 'html',
        value: `<details data-autogenerated-from="${detailsPath}">`,
      } as any);
  ...
  for (const page of pages) {
    ...
    if (hasKeywords || hasSections || hasParts || hasExports || hasTypes) {
      const detailsPath = page.path.endsWith('/page.mdx')
        ? page.path.substring(0, page.path.length - '/page.mdx'.length)
        : page.path;
      lines.push(`<details data-autogenerated-from="${detailsPath}">`);
      lines.push('');
