...
export function generateTypesMarkdown(name: string, types: TypesMeta[]): string {
  ...
  types.forEach((typeMeta, idx) => {
    if (typeMeta.type === 'component') {
      ...
      // For other types, we might want to show basic type information
      // This could be extended based on what information is useful for ExportNode types
    }

    // Add separator between parts
    if (types.length > 1 && idx < types.length - 1) {
      tables.push(md.paragraph(''));
    }
  });
