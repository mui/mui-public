// The transform-application core. Split out of `applyCodeTransform` so the
// decode step is *injected* rather than statically imported: the client-side
// transform engine (`useCode/TransformEngine`) reaches this through a dynamic
// `import()` and passes the shell's already-loaded `decodeHastSource`, so the
// engine chunk never statically pulls `decodeHastSource` (and its
// `hastDecompress` dependency) — those stay counted in the `useCode` shell
// instead of being hoisted into their own chunks. The standalone server/build
// pipeline keeps its original API through the thin `applyCodeTransform` wrapper,
// which binds `decodeHastSource` for callers that don't inject one.
 ...
 */
function regenerateMissingFrameFallbacksInPlace(
  root: Nodes,
  frameFallbackFromSpans: FrameFallbackFromSpans,
): void {
  if (root.type !== 'root') {
...
export function applyCodeTransformWithComments(
  ...
  // lives as a tree.
  const sourceRoot = deps.decode(source, fallback);
  if (!sourceRoot) {
  ...
  regenerateMissingFrameFallbacksInPlace(patchedRoot, deps.frameFallbackFromSpans);
...
export function applyCodeTransformsWithComments(
  ...
  if (transformKeys.length > 1 && typeof source !== 'string') {
    const sourceRoot = deps.decode(source, fallback) as HastRoot | undefined;
    const embeddedTransforms = sourceRoot?.data?.transforms;
