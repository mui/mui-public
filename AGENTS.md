...
- **3.5** Avoid mocks in unit tests. Use real implementations whenever possible to ensure tests are reliable and maintainable. This is about what you fake, not which tool you fake it with — dependency injection is still a mock, so adding a `deps` parameter or a hand-rolled interface that only tests supply does not satisfy this rule. Never fake internals: a seam that exists only for tests couples them to the current shape of the code and has to be rewritten on every refactor. Fake only genuine externals (network, database, clock), and only when there is no way to avoid it — prefer extracting the logic into a pure function that takes data and needs no fake at all. When you must fake an external library, do it so that a change to that library's TypeScript API breaks typechecking: keep the real client in the test's path and substitute at its own boundary (for example construct a real `Octokit` with `request: { fetch }` serving canned HTTP responses) rather than hand-writing a narrow interface or casting a partial object with `as any`, both of which silently drift from upstream.
