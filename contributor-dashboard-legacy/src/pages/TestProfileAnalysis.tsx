...
function ProfilerInteractions(props: { interactions: { id: number; name: string }[] }) {
  const testProfileDetails = useTestProfileDetails();

  const interactions = props.interactions.map((interaction) => {
    const traceByStackMatch = interaction.name.match(/^([^:]+):(\d+):\d+ \(([^)]+)\)$/);
    if (traceByStackMatch === null) {
      // eslint-disable-next-line no-console
      console.log(interaction.name);
      const unknownLineMatch = interaction.name.match(/^unknown line \(([^)]+)\)$/);
      return <ListItem key={interaction.id}>{unknownLineMatch?.[1] ?? interaction.name}</ListItem>;
    }
    ...
    if (testProfiles?.length > 0) {
      ...
      if (profilesByBrowserName[browserName] === undefined) {
        ...
        testProfiles.forEach((testProfile, interactionIndex) => {
          const interaction = profilesByBrowserName[browserName][interactionIndex];
                      ...
                      {renders.map((render, interactionIndex) => {
                        return (
                          <tr key={interactionIndex}>
                            ...
                            <td align="right" style={{ fontVariantNumeric: 'tabular-nums' }}>
                              <TimingAnalysisMean format={formatMs} timings={render.baseDuration} />
                            </td>
                            <td>
                              <ProfilerInteractions interactions={render.interactions} />
                            </td>
                          </tr>
                        );
                      })}
...
async function fetchTestProfileArtifactsInfos(
  ...
  return infos
    ...
    .filter((maybeTestProfileArtifact): maybeTestProfileArtifact is TestProfileArtifactsInfo => {
      return maybeTestProfileArtifact !== null;
    });
}

function useTestProfileArtifactsInfos(buildNumber: number): TestProfileArtifactsInfo[] {
...
function ProfiledTests() {
  ...
  return (
    <React.Fragment>
      <Heading level="2">
        Tests for{' '}
        <Link href={profileDetails.reviewUrl} rel="noopener noreferrer" target="_blank">
