...
Store logs outside the checkout. The `code-infra` CLI authenticates GitHub Actions requests through
the built-in Octokit helper.
...
```bash
pnpm code-infra ci-logs gh-action mui/example 123456 /tmp/mui-job-123456.log
pnpm code-infra ci-logs circleci mui/example 7890 /tmp/mui-job-7890.log
```

GitHub uses the built-in Octokit authentication; the CircleCI helper uses the v1.1 job API, anonymously for public
...
```bash
pnpm code-infra ci-logs netlify OWNER/REPO DEPLOY_ID OUTPUT --head PR_HEAD_SHA
```
